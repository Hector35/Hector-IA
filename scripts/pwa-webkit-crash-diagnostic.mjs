...
const cases=[
  ...
  {id:'app-core-only',javaScriptEnabled:true,block:req=>req.resourceType()==='script'&&!/\/(app-v16|progressive-photo-queue-v45)\.js(?:\?|$)/.test(new URL(req.url()).pathname+new URL(req.url()).search),description:'Only app-v16 + progressive photo queue scripts'},
  {id:'no-interaction-extensions',javaScriptEnabled:true,block:req=>req.resourceType()==='script'&&/\/(interaction-runtime-v85|capture-ios-v88|capture-fix-v80|patient-detail-history-v82|e2e-v74|runtime-hardening-v86)\.js(?:\?|$)/.test(new URL(req.url()).pathname+new URL(req.url()).search),description:'Full app except interaction/capture/detail/e2e extension scripts'}
];
...
async function runCase(testCase){
  ...
  const result={id:testCase.id,description:testCase.description,ok:false,crashed:false,elapsedMs:0};
  try{
    ...
    const state=await Promise.race([
      ...
      sleep(6000).then(()=>{throw new Error('state evaluation timeout');})
    ]);
    result.state=state;
    result.ok=true;
  }catch(error){
...
const report={auditedAt:new Date().toISOString(),target,results};
