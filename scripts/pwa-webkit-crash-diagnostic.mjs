...
const scriptName=req=>{try{return new URL(req.url()).pathname.split('/').pop()?.replace(/\.js$/,'')||'';}catch{return '';}};
const allowScripts=(ids,serviceWorkers='allow',id='custom',description='')=>({
  id,description,javaScriptEnabled:true,serviceWorkers,
  block:req=>req.resourceType()==='script'&&!ids.includes(scriptName(req))&&!/^progressive-photo-queue-v45$/.test(scriptName(req))
});
const prefix=n=>orderedScripts.slice(0,n);
const cases=[
  allowScripts(prefix(10),'allow','prefix-10-baseline','Known-good prefix through capture-ios-v88'),
  allowScripts(prefix(11),'allow','prefix-11-patient-detail','Add patient-detail-history-v82 only'),
  allowScripts([...prefix(10),'interaction-runtime-v85'],'allow','prefix-10-plus-interaction','Add interaction-runtime-v85 without patient detail'),
  allowScripts(prefix(12),'allow','prefix-12','Patient detail + interaction runtime'),
  allowScripts(prefix(12),'block','prefix-12-no-sw','Same prefix with service workers blocked'),
...
async function runCase(testCase){
  ...
  const result={id:testCase.id,description:testCase.description,serviceWorkers:testCase.serviceWorkers,ok:false,crashed:false,responsive:false,elapsedMs:0,requests:[]};
  try{
...
const results=[];for(const testCase of cases)results.push(await runCase(testCase));
