(() => {
  ...
  function markRealizado(id) {
    ...
    document.dispatchEvent(new CustomEvent('pendientes:status-changed',{detail:{id:String(id),status:'Realizado',source:'TAC-fallback'}}));
  }
  ...
  document.addEventListener('touchend',(event)=>{if(!gesture||event.changedTouches.length!==1){gesture=null;return;}const touch=event.changedTouches[0],dx=touch.clientX-gesture.x,dy=touch.clientY-gesture.y,row=gesture.row;gesture=null;if(dx < -72&&Math.abs(dx)>Math.abs(dy)*1.4&&window.__PENDIENTES_GLOBAL_STATUS_GESTURES__ !== true)markRealizado(row.dataset.id);},{passive:true});
