import { syncRowsFromStorageAndRender, canonicalOrigin, normalizeAge, normalizeStudyDisplay } from './app-v16.js?v=65';
...
Reglas: el total del encabezado NO crea pacientes. Recorre cada renglón físico de ambas columnas. H=Hombre, M=Mujer por columna. Conserva CE/UP/UI literalmente. Si ves un renglón pero la cama no es legible, devuelve un parcial con bed vacío y recognizedText. Nunca inventes camas para alcanzar el total.`;
...
function toast(msg,tone=''){document.getElementById('v79Toast')?.remove();const el=document.createElement('div');el.id='v79Toast';el.className=`v75-toast ${tone}`;el.textContent=msg;document.body.appendChild(el);setTimeout(()=>el.remove(),3000)}
...
function transportOf(p,target){
  ...
  const explicitNoMove=/\b(no trasladar|no se traslada|sin traslado)\b/.test(e);
  const portable=/\bportatil\b/.test(targetText)||/\bportatil\b/.test(e);
  ...
  const explicitCamilla=/\b(camilla|en camilla|traslado en camilla)\b/.test(e);
  const strongCamilla=/\b(inmovil|inmovilidad|no deambula|no puede deambular|encamado|hemiplej|paraplej|tetraplej|deficit neurologico motor|fractura desplazada|fractura de femur|fractura de cadera|trauma grave|estado general delicado)\b/.test(e);
...
function normalizeRow(p,fp,shiftId,{partial=false,imageAvailable=true}={}){
  const category=categoryOf(p),bed=normalizeBed(p?.handwrittenBed)||normalizeBed(p?.formBed)||normalizeBed(p?.bed),rawTarget=clean(p?.target||p?.study||p?.destination),target=category==='Piso'?clean(p?.destination||rawTarget):normalizeStudyDisplay(rawTarget),[transport,transportReason]=transportOf(p,target);
  ...
  const row={...p,id:uid(),shiftId,category,bed,name:clean(p?.name),birthDate:clean(p?.birthDate),age:normalizeAge(p?.age)??ageFromBirth(p?.birthDate),sex:normSex(p?.sex),target,destination:category==='Piso'?clean(p?.destination||target):clean(p?.destination),destinationFloor:clean(p?.destinationFloor),destinationBlock:clean(p?.destinationBlock),destinationService:clean(p?.destinationService||((category==='Piso'&&!p?.originService)?p?.service:'')),modality:category==='TAC'?'TAC':category==='USG'?'Ultrasonido':category==='RX'?'Rayos X':clean(p?.modality)||'Otro',diagnosis:clean(p?.diagnosis),diagnosisMeaning:clean(p?.diagnosisMeaning),requestingDoctor:clean(p?.requestingDoctor||p?.doctor),folio:clean(p?.folio),service:category==='Piso'?clean(p?.originService):clean(p?.service),originService:clean(p?.originService),requestDate:clean(p?.requestDate),requestTime:clean(p?.requestTime),transferNotes:clean(p?.transferNotes),extraData:p?.extraData&&typeof p.extraData==='object'?p.extraData:{},recognizedText:clean(p?.recognizedText),transport,transportReason,oxygenProbable:Boolean(p?.oxygenProbable),oxygenReason:p?.oxygenProbable?clean(p?.oxygenReason):'',status:'Pendiente',imageFingerprint:fp,boletaImageFingerprint:fp,boletaImageAvailable:imageAvailable,confidence,needsReview:partial||reviewFields.length>0,reviewFields,captureReviewOnly:partial,manualOverrides:{},createdAt:now(),updatedAt:now()};
...
function dedupePatients(patients){const out=[];for(const [n,p] of (patients||[]).entries()){const category=categoryOf(p),bed=normalizeBed(p?.handwrittenBed)||normalizeBed(p?.formBed)||normalizeBed(p?.bed);const key=category==='Piso'&&bed?`Piso:${canonicalOrigin(bed)}`:category==='Piso'?`Piso:partial:${plain(p?.recognizedText)}:${plain(p?.destinationService||p?.service)}:${plain(p?.name)}:${n}`:`${category}:${plain(p?.name)}:${plain(p?.target||p?.destination)}`;const i=out.findIndex(x=>x.key===key);if(i<0)out.push({key,p});else out[i].p={...out[i].p,...p,recognizedText:clean(p?.recognizedText)||clean(out[i].p?.recognizedText)}}return out.map(x=>x.p)}
function commitPatients(patients,fp,shiftId,imageAvailable){
  ...
  for(const p of dedupePatients(Array.isArray(patients)?patients:[])){
    ...
    if(provisional.category==='Piso'&&!provisional.bed){
      ...
      if(provisional.name||provisional.destinationService||provisional.service||provisional.recognizedText){const partial=normalizeRow(p,fp,shiftId,{partial:true,imageAvailable});const duplicatePartial=list.find(r=>r.shiftId===shiftId&&r.captureReviewOnly&&r.imageFingerprint===fp&&plain(r.recognizedText)===plain(partial.recognizedText)&&plain(r.destinationService||r.service)===plain(partial.destinationService||partial.service));if(!duplicatePartial){list.unshift(partial);partials++}}
      continue;
    }
  ...
  write(STORAGE_KEY,list);syncRowsFromStorageAndRender();window.dispatchEvent(new CustomEvent('pendientes:v79-updated'));return{added,updated,partials,skipped};
}
...
async function saveImage(fp,file){try{const d=await db();await new Promise((resolve,reject)=>{const tx=d.transaction(STORE,'readwrite');tx.objectStore(STORE).put({fp,blob:file,name:file.name||'boleta.jpg',type:file.type||'image/jpeg',createdAt:now()});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});d.close();return true}catch(e){console.warn('[Pendientes v79] No se pudo guardar foto de boleta',e);return false}}
async function deleteImage(fp){try{const d=await db();await new Promise((resolve,reject)=>{const tx=d.transaction(STORE,'readwrite');tx.objectStore(STORE).delete(fp);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});d.close()}catch(e){console.warn('[Pendientes v79] No se pudo limpiar foto huérfana',e)}}
...
function floorRecognitionStats(patients){let confirmedBeds=0,partialRows=0;for(const p of patients||[]){if(categoryOf(p)!=='Piso'||isHeaderOnly(p))continue;const bed=normalizeBed(p?.handwrittenBed)||normalizeBed(p?.formBed)||normalizeBed(p?.bed);if(bed)confirmedBeds++;else if(clean(p?.recognizedText)||clean(p?.name)||clean(p?.destinationService||p?.service))partialRows++}return{confirmedBeds,partialRows}}
...
async function analyzeFile(file,index,total){
  ...
  try{
    ...
    if(shouldRescan){setProgress(boundedTotal?`Revisando renglones de Piso (${confirmedBeds} camas + ${partialRows} parciales / ${boardTotal})…`:'Revisando renglones de Piso…');try{const second=await callVision(file,FLOOR_RESCAN_PROMPT),extra=Array.isArray(second?.patients)?second.patients:[];patients=[...patients,...extra]}catch(e){console.warn('[Pendientes v79] Segunda lectura de Piso no disponible',e)}}
    const result=commitPatients(patients,fp,shift().id,imageAvailable);
    if(imageAvailable&&result.added===0&&result.updated===0&&result.partials===0&&!referencesImage(fp))await deleteImage(fp);
    return result;
  }catch(e){if(imageAvailable&&!referencesImage(fp))await deleteImage(fp);throw e}
}

let processing=false;
async function processFiles(files){if(processing)return;const arr=[...(files||[])];if(!arr.length)return;processing=true;let added=0,updated=0,errors=0,partials=0,skipped=0;for(let i=0;i<arr.length;i++){try{const r=await analyzeFile(arr[i],i,arr.length);added+=r.added;updated+=r.updated;partials+=r.partials;skipped+=r.skipped}catch(e){errors++;toast(e?.message||'No se pudo analizar una foto','warning')}}processing=false;setProgress('');toast(`${added} agregados${updated?` · ${updated} actualizados`:''}${partials?` · ${partials} por revisar`:''}${skipped?` · ${skipped} omitidos`:''}${errors?` · ${errors} error${errors===1?'':'es'}`:''}`,errors?'warning':'success')}
...
window.addEventListener('pendientes:v79-updated',()=>queueMicrotask(()=>document.documentElement.dataset.pendientesBuild=BUILD));
