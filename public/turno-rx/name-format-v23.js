...
export function formatPatientName(value) {
  const source = clean(value);
  if (!source || source === '—') return '—';

  const upper = source.toLocaleUpperCase('es-MX');

  // Expedientes que vienen como "APELLIDO(S), NOMBRE(S)".
  if (upper.includes(',')) {
  ...
  // Si ya empieza con nombre propio, conserva el orden.
  if (isGiven(tokens[0])) return upper;

  // Evita confundir DE JESÚS / DEL ... como nombre cuando forma parte del apellido.
  ...
  // Si no hay señal confiable, no inventa: únicamente MAYÚSCULAS.
  return upper;
}

export function applyPatientNameFormat(root = document) {
...
function schedule() {
  ...
  queueMicrotask(() => {
    scheduled = false;
    applyPatientNameFormat(document);
  });
}
