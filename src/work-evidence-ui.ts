...
export function installWorkEvidenceEnhancer(){
 ...
 const showJob=async(detail:HTMLElement,job:WorkJob)=>{
  ...
  const input=document.createElement('input');input.type='url';input.required=true;input.autocomplete='url';input.placeholder='https://...';input.value=location.origin;input.setAttribute('aria-label','URL pública HTTPS a verificar');
