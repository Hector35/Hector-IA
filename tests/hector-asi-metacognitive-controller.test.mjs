...
import { compareApproaches, evaluateIncrementally, selectiveController } from '../model/hector-asi/metacognitive_controller.mjs';
...
describe('Hector ASI selective metacognitive controller', () => {
  ...
  it('reuses unchanged benchmark rows and recomputes only a changed output', () => {
    ...
    const changed = { ...outputs, [cases[0].id]: { ...outputs[cases[0].id], confidence: outputs[cases[0].id].confidence - 0.01 } };
