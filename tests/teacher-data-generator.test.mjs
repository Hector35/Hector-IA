...
describe('OpenAI teacher data pipeline',()=>{
 ...
 it('accepts only public prompt inputs with review criteria',()=>{
  ...
  for(const row of rows){expect(row.license).toBe('CC0-1.0');expect(row.containsPrivateUserData).toBe(false);expect(row.expectedSignals.length).toBeGreaterThanOrEqual(2);}
 });
});
