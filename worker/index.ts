...
import {bridgeSecurity} from './bridge-security';
...
import {agentProgrammingRunner,processProgrammingAgentJobs} from './routes/agent-programming-runner';
...
app.route('/control/v1',control);app.route('/generated',generated);app.route('/self-improve/v1',selfImprove);app.route('/runner/v1',runner);app.route('/runner/v1',pwaRunnerStatus);app.route('/runner/v2',agentProgrammingRunner);app.route('/evidence/v1',evidence);app.route('/api/auth',auth);app.route('/api/agent',agent);app.route('/api/intelligence',selfModel);app.route('/api/intelligence/budget',cognitiveBudget);app.route('/api/intelligence/benchmark',intelligenceBenchmark);app.route('/api/intelligence/plan-incidents',planDriftIncidents);app.route('/api/intelligence',openaiCoach);app.route('/api/intelligence',qwen397Chat);app.route('/api/intelligence',kimiChat);app.route('/api/intelligence',modelChat);app.route('/api/intelligence',intelligence);app.route('/api/notifications',operationalNotifications);app.route('/api/work-mode',workMode);app.route('/api/hector-agent/resilience',hectorAgentResilience);app.route('/api/hector-agent',hectorAgent);app.route('/api/system',systemInfo);app.route('/api/action-authority',actionAuthority);app.route('/api/delegations',delegations);app.route('/api/projects',projectGovernance);app.route('/api/projects',projects);app.route('/api/pwa-factory',pwaFactory);app.route('/api/conversations',conversations);app.route('/api/chat-agents',chatAgents);app.route('/api/work-evidence',workEvidence);app.route('/api/memory',memoryStatus);app.route('/api/provider-health',providerHealth);app.route('/api/response-traces',responseTraces);app.route('/api/schedules',schedules);app.route('/api',qwen397Vision);app.route('/api',intelligence);app.route('/api',openInteraction);app.route('/api',api);
...
async function claimNextJob(env:Bindings){
 ...
 const job=await env.DB.prepare(`UPDATE work_jobs SET status='working',progress=10,attempt_count=attempt_count+1,lease_token=?,lease_expires_at=datetime('now',?),heartbeat_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
  WHERE id=(SELECT id FROM work_jobs WHERE status='queued' AND kind!='programming' AND (next_retry_at IS NULL OR next_retry_at<=CURRENT_TIMESTAMP)
   AND NOT EXISTS(SELECT 1 FROM hector_agent_goals hg JOIN hector_agent_settings hs ON hs.user_id=hg.user_id WHERE hg.work_job_id=work_jobs.id AND (hs.paused=1 OR hs.auto_enabled=0))
   ORDER BY created_at LIMIT 1) AND status='queued' RETURNING *`).bind(leaseToken,`+${JOB_LEASE_SECONDS} seconds`).first<any>();
...
async function processNext(env:Bindings){
 ...
 const initialGuard=await loadHectorAgentRuntimeGuard(env,job.id);
 if(initialGuard){
  const reason=hectorAgentLimitReason(initialGuard,job.attempt_count,'before');
  if(reason){const blocked=await blockHectorAgentJob(env,{workJobId:job.id,leaseToken,reason});if(blocked.meta.changes)await event(env,job.id,`Ejecución detenida antes de iniciar: ${reason}`,job.progress||0);return;}
 }
 ...
 try{
  ...
  const {out,plan:executionPlan,verification,usage:u}=execution;cycleCostUsd=Math.max(0,Number(u.costUsd)||0);
  ...
  if(!verification.ok){await recordUsage(env,job,out,u,executionPlan,verification,'background-policy-drift-blocked');throw new Error(`Ejecución bloqueada por desviación del plan cognitivo: ${verification.differences.join('; ')}`);}
  const budgetMessage=budgetEvent(out.budgetDecision);if(budgetMessage)await event(env,job.id,budgetMessage,65);
  const runtimeGuard=initialGuard?await recordHectorAgentCycle(env,job.id,{durationMs:Date.now()-started,costUsd:cycleCostUsd,failed:false}):null;
  if(continuous){
   const parsed=parseWorkModeResult(out.text);
   if(parsed.state!=='complete'){
    const limitReason=runtimeGuard?hectorAgentLimitReason(runtimeGuard,job.attempt_count,'after'):null;
    if(limitReason){
     await recordUsage(env,job,out,u,executionPlan,verification,'hector-agent-limit');
     const blocked=await blockHectorAgentJob(env,{workJobId:job.id,leaseToken,reason:limitReason,result:parsed.text});
     if(blocked.meta.changes){await event(env,job.id,`Héctor Agent se detuvo de forma segura: ${limitReason}`,Math.min(99,Math.max(10,job.progress||10)));await recordExperience(env,{jobId:job.id,userId:job.user_id,objective:job.prompt,status:'blocked',result:limitReason,skills:plan.skills,attempts:job.attempt_count,durationMs:Date.now()-started});}
     return;
    }
  ...
  const failureGuard=initialGuard?await recordHectorAgentCycle(env,job.id,{durationMs:Date.now()-started,costUsd:cycleCostUsd,failed:true}):null;
  if(continuous){
...
export default {fetch:app.fetch,scheduled:(_controller:ScheduledController,env:Bindings,ctx:ExecutionContext)=>{ctx.waitUntil(processScheduledWork(env));ctx.waitUntil(processProgrammingAgentJobs(env));ctx.waitUntil(processNextDelegation(env));ctx.waitUntil(processProjectQueue(env));ctx.waitUntil(processChatAgentRuns(env));}};