...
async function processNext(env:Bindings){
 ...
 try{
  ...
  const out=await respond(env,`Ejecuta según el protocolo y entrega evidencia por criterio.\n\n${job.prompt}`,undefined,[],Boolean(job.allow_web??1),{reasoning:high?'high':'auto',deliberation:high?'force':'auto'}),u=estimateCost(out.usage);if(out.searchedWeb)u.costUsd+=.01;
