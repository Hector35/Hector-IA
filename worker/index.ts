...
import {cleanWorkModeOutput,isPersistentWorkPrompt,nextWorkModeDelayMinutes,parseWorkModeOutcome,unwrapWorkObjective,workModeExecutionPrompt} from './lib/work-mode';
...
async function processNext(env:Bindings){
 ...
 const {job,leaseToken}=claimed,started=Date.now(),persistent=isPersistentWorkPrompt(job.prompt)&&!job.schedule_id,objective=persistent?unwrapWorkObjective(job.prompt):job.prompt,plan=buildPlan(objective);await event(env,job.id,persistent?`Modo Trabajo retomado. Ciclo ${job.attempt_count}.`:`Inspección iniciada. Intento ${job.attempt_count}/${job.max_attempts}. Skills: ${plan.skills.join(', ')||'general-analysis'}`,10);
 try{
  const high=persistent||job.reasoning_level==='high',prompt=persistent?workModeExecutionPrompt(job.prompt,job.result,job.attempt_count):`Ejecuta según el protocolo y entrega evidencia por criterio.\n\n${job.prompt}`;
  ...
  const execution=await executePlannedWork(env,{userId:job.user_id,prompt,allowWeb:Boolean(job.allow_web??1),reasoningLevel:high?'high':'auto',context:persistent?`Modo Trabajo persistente ${job.id}. Ciclo ${job.attempt_count}. Continúa hasta completar o quedar en espera real.`:`Trabajo ${job.id}. Tipo: ${job.kind}. Intento ${job.attempt_count}/${job.max_attempts}.`});
  ...
  if(persistent){
   const outcome=parseWorkModeOutcome(out.text),checkpoint=cleanWorkModeOutput(out.text);
   if(outcome.state==='completed'){
    const completed=await env.DB.prepare("UPDATE work_jobs SET status='completed',progress=100,result=?,last_error=NULL,lease_token=NULL,lease_expires_at=NULL,next_retry_at=NULL,heartbeat_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND lease_token=?").bind(checkpoint,job.id,leaseToken).run();if(!completed.meta.changes)return;
    await event(env,job.id,'Objetivo completado con evidencia. Modo Trabajo finalizado.',100);
    await recordPlannedUsage(env,job,out,u,executionPlan,verification,'work-mode-cycle-planned');
    await recordExperience(env,{jobId:job.id,userId:job.user_id,objective,status:'completed',result:checkpoint,skills:plan.skills,attempts:job.attempt_count,durationMs:Date.now()-started});return;
   }
   const delay=nextWorkModeDelayMinutes(outcome,job.attempt_count),message=outcome.state==='waiting'?`Dependencia real detectada. Modo Trabajo queda en espera ${delay} min y reanudará automáticamente.`:`Avance guardado. Modo Trabajo continuará automáticamente en ${delay} min.`;
   const queued=await env.DB.prepare(`UPDATE work_jobs SET status='queued',progress=50,result=?,last_error=NULL,next_retry_at=datetime('now','+${delay} minutes'),lease_token=NULL,lease_expires_at=NULL,heartbeat_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND lease_token=?`).bind(checkpoint,job.id,leaseToken).run();if(!queued.meta.changes)return;
   await event(env,job.id,message,50);await recordPlannedUsage(env,job,out,u,executionPlan,verification,'work-mode-cycle-planned');await recordExperience(env,{jobId:job.id,userId:job.user_id,objective,status:'queued',result:checkpoint,skills:plan.skills,attempts:job.attempt_count,durationMs:Date.now()-started});return;
  }
  ...
  await env.DB.batch([env.DB.prepare("INSERT INTO work_events(id,job_id,message,progress) VALUES(?,?, 'Resultado guardado bajo plan cognitivo verificado',100)").bind(crypto.randomUUID(),job.id)]);await recordPlannedUsage(env,job,out,u,executionPlan,verification,job.schedule_id?'scheduled-work-planned':'background-work-planned');await recordExperience(env,{jobId:job.id,userId:job.user_id,objective:job.prompt,status:'completed',result:out.text,skills:plan.skills,attempts:job.attempt_count,durationMs:Date.now()-started});
 }catch(e){
  const message=e instanceof Error?e.message:'Error';
  if(persistent){const delay=retryDelaySeconds(job.attempt_count),failed=await env.DB.prepare(`UPDATE work_jobs SET status='queued',last_error=?,next_retry_at=datetime('now','+${delay} seconds'),lease_token=NULL,lease_expires_at=NULL,heartbeat_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND lease_token=?`).bind(message,job.id,leaseToken).run();if(failed.meta.changes){await event(env,job.id,`Error detectado. Modo Trabajo buscará otra alternativa en ${delay}s; no se detuvo.`,25);await recordExperience(env,{jobId:job.id,userId:job.user_id,objective,status:'queued',result:message,skills:plan.skills,attempts:job.attempt_count,durationMs:Date.now()-started});}return;}
