...
async function recoverExpiredLeases(env:Bindings){
 ...
 WHERE status='working' AND (lease_expires_at IS NULL OR lease_expires_at<=CURRENT_TIMESTAMP)`).run();
}
...
async function claimNextJob(env:Bindings){
 ...
 WHERE id=(SELECT id FROM work_jobs WHERE status='queued' AND (next_retry_at IS NULL OR next_retry_at<=CURRENT_TIMESTAMP) ORDER BY created_at LIMIT 1)
 AND status='queued'
