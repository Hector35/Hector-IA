...
function budgetEvent(decision:any){if(!decision)return null;return decision.action==='degrade'?`Presupuesto cognitivo aplicado: ruta de menor costo (${decision.dailyPercent}% diario, ${decision.monthlyPercent}% mensual)`:`Presupuesto cognitivo: ${decision.action} (${decision.state})`;}
async function processNext(env:Bindings){
 ...
 try{
  ...
  await event(env,job.id,high?'Plan cargado; usando GPT-5.6 Sol con deliberación multiagente':'Plan verificable cargado; preparando ejecución',25);
  const out=await respond(env,`Ejecuta según el protocolo y entrega evidencia por criterio.\n\n${job.prompt}`,undefined,[],Boolean(job.allow_web??1),{reasoning:high?'high':'auto',deliberation:high?'force':'auto',userId:job.user_id}),u=estimateCost(out.usage,out.model);if(out.searchedWeb)u.costUsd+=.01;
  const budgetMessage=budgetEvent(out.budgetDecision);if(budgetMessage)await event(env,job.id,budgetMessage,35);
