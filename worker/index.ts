...
async function processNext(env:Bindings){
 ...
 try{
  ...
  await event(env,job.id,forced?'Plan cargado; usando GPT-5.6 Sol con deliberación y razonamiento máximo':'Plan verificable cargado; preparando ejecución',25);
  const out=await respond(env,`Ejecuta según el protocolo y entrega evidencia por criterio.\n\n${job.prompt}`,undefined,[],Boolean(job.allow_web??1),{reasoning:forced?'high':'auto',deliberation:forced?'force':'auto'}),u=estimateCost(out.usage,out.model);if(out.searchedWeb)u.costUsd+=.01;
  ...
  await env.DB.batch([env.DB.prepare("INSERT INTO work_events(id,job_id,message,progress) VALUES(?,?, 'Resultado guardado con evidencia y límites declarados',100)").bind(crypto.randomUUID(),job.id),env.DB.prepare('INSERT INTO api_usage(id,user_id,provider,service,model,input_units,cached_input_units,output_units,estimated_cost_usd,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(crypto.randomUUID(),job.user_id,out.provider==='cloudflare'?'Cloudflare':'OpenAI',job.schedule_id?'scheduled-work-v2':'background-work-v3',out.model,u.input,u.cached,u.output,u.costUsd,JSON.stringify({tier:out.modelTier,reasoningEffort:out.reasoningEffort,modelReason:out.modelReason,searchedWeb:out.searchedWeb,cognitiveMode:out.cognitiveMode,deliberationPasses:out.deliberationPasses,deliberationReason:out.deliberationReason}))]);
