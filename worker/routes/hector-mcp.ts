...
type ToolDef={name:string;description:string;scope:string;method:'GET'|'POST';path:string;target:'bridge'|'context'|'sync'|'capabilities';inputSchema:Record<string,unknown>;wrap?: (args:any)=>any};
...
function subApp(tool:ToolDef){return tool.target==='bridge'?hectorBridge:tool.target==='context'?contextHub:tool.target==='sync'?contextSync:hectorCapabilities;}
...
hectorMcp.post('/',async c=>{
  ...
  if(method==='initialize'){
    ...
    return c.json(jsonRpc(id,{protocolVersion:requested,capabilities:{tools:{listChanged:false}},serverInfo:{name:'hector-bridge',version:'2.0.0'},instructions:'Use Context Sync before substantial work, execute capabilities through the broker, and publish meaningful handoffs. Coordination is advisory, not an internal permission gate.'}));
  }
