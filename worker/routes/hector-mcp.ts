...
const TOOLS:ToolDef[]=[
 ...
 {name:'apple_shortcut_create',description:'Diseña con ChatGPT un Atajo de Apple y devuelve la receta exacta, el enlace para abrir el editor y el enlace de ejecución. iOS siempre conserva la confirmación final de acciones y permisos.',scope:'tools',method:'POST',path:'/shortcuts/design',target:'bridge',inputSchema:{type:'object',properties:{name:{type:'string'},purpose:{type:'string'},input:{type:'string',enum:['none','text','clipboard','share_sheet']},actions:{type:'array',minItems:1,maxItems:20,items:{type:'object',properties:{name:{type:'string'},configuration:{type:'string'}},required:['name'],additionalProperties:false}}},required:['name','purpose','actions'],additionalProperties:false}},
