...
async function repairWithProvider(env:Bindings,providerKind:ProviderKind,system:string,history:Array<{role:'user'|'assistant';content:string}>,prompt:string){if(providerKind==='qwen397')return callQwen397(env,`${system}\n\nMODO DE REPARACIÓN COGNITIVA: corrige únicamente los criterios fallidos.`,[...history,{role:'user',content:prompt}]);if(providerKind==='kimi')return callKimiK2_5(env,`${system}\n\nMODO DE REPARACIÓN COGNITIVA: corrige únicamente los criterios fallidos.`,[...history,{role:'user',content:prompt}]);return callCloudflare(env,`${system}\n\nMODO DE REPARACIÓN COGNITIVA: corrige únicamente los criterios fallidos.`,prompt);}
...
qwen397Chat.post('/qwen397-chat',async c=>{
 ...
 try{
  ...
  if(!verification.accepted&&runtimePlan.maxAttempts>1){try{const repairPrompt=buildCognitiveRepairPrompt({prompt:message,draft:out.text,verification,plan:runtimePlan}),repaired=await repairWithProvider(c.env,providerKind,system,history,repairPrompt);out={...repaired,usage:combineUsage(out.usage,repaired.usage),latencyMs:Number(out.latencyMs||0)+Number((repaired as any).latencyMs||0)};verification=verifyCognitiveResponse({prompt:message,text:out.text,plan:runtimePlan,searchedWeb:false});attempts.push({attempt:2,phase:'repair',provider,model:out.model,verification});reason=`${reason}; reparación cognitiva ejecutada`;}catch(error){reason=`${reason}; reparación cognitiva no disponible: ${error instanceof Error?error.message:'error desconocido'}`;}}
