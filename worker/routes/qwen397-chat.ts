...
function instructions(context:string){return `${renderBootstrap()}\n\nRUNTIME PRINCIPAL\n- Modelo solicitado: Qwen/Qwen3.5-397B-A17B.\n- MoE multimodal: 397B totales, 17B activos.\n- No afirmes que respondió Qwen si el runtime reporta fallback.\n- No afirmes pesos personalizados hasta que un checkpoint supere Benchmark V2.\n- Responde en español salvo petición contraria.\n- Distingue hechos, inferencias, acciones y límites.\n\nCONTEXTO DINÁMICO\n${context}`;}
async function fallbackAfterQwen(env:Bindings,system:string,history:Array<{role:'user'|'assistant';content:string}>,message:string,baseReason:string):Promise<FallbackResult>{if(hasKimiEndpoint(env)){try{const out=await callKimiK2_5(env,`${system}\n\nQwen no estuvo disponible. Responde como fallback Kimi identificado.`,history);return{out,provider:'Kimi K2.5 endpoint',fallbackLevel:1,reason:`${baseReason}; se usó Kimi K2.5`};}catch(error){const kimiError=error instanceof Error?error.message:'error desconocido';const out=await callCloudflare(env,`${system}\n\nQwen y Kimi no estuvieron disponibles. Responde como fallback Workers AI identificado.`,message);return{out,provider:'Cloudflare Workers AI',fallbackLevel:2,reason:`${baseReason}; Kimi falló: ${kimiError}; se usó Workers AI`};}}const out=await callCloudflare(env,`${system}\n\nQwen no estuvo disponible y Kimi no está configurado. Responde como fallback Workers AI identificado.`,message);return{out,provider:'Cloudflare Workers AI',fallbackLevel:2,reason:`${baseReason}; Kimi no está configurado; se usó Workers AI`};}
...
qwen397Chat.post('/qwen397-chat',async c=>{
 ...
 const message=parsed.data.message.trim(),userId=c.get('userId'),conversationId=await ensureConversation(c.env.DB,userId,parsed.data.conversationId,message);
 ...
 const pack=await loadContextPack(c.env,userId,conversationId,message),system=instructions(renderContext(pack)),history=pack.recentMessages.filter(item=>item.role==='user'||item.role==='assistant').map(item=>({role:item.role as 'user'|'assistant',content:item.content}));
 const qwenConfigured=hasQwen397Endpoint(c.env),kimiConfigured=hasKimiEndpoint(c.env);let out:RoutedOutput;let provider='Qwen 397B endpoint',fallback=false,reason='Qwen 397B respondió con contenido válido',fallbackLevel:0|1|2=0;
 try{
  if(qwenConfigured){try{out=await callQwen397(c.env,system,history);}catch(error){const routed=await fallbackAfterQwen(c.env,system,history,message,`Qwen falló: ${error instanceof Error?error.message:'error desconocido'}`);out=routed.out;provider=routed.provider;fallback=true;fallbackLevel=routed.fallbackLevel;reason=routed.reason;}}
  else{const routed=await fallbackAfterQwen(c.env,system,history,message,'Qwen no tiene endpoint y secreto configurados');out=routed.out;provider=routed.provider;fallback=true;fallbackLevel=routed.fallbackLevel;reason=routed.reason;}
  const contract=enforceResponseContract(message,out.text),assistantMessageId=crypto.randomUUID(),status=qwen397Status(c.env),effectiveQwen=!fallback&&qwenConfigured;
  await c.env.DB.batch([
   ...
   c.env.DB.prepare('INSERT INTO api_usage(id,user_id,provider,service,model,input_units,cached_input_units,output_units,estimated_cost_usd,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(crypto.randomUUID(),userId,provider,'qwen397-chat',out.model,Number(out.usage?.input_tokens||0),0,Number(out.usage?.output_tokens||0),0,JSON.stringify({runtimeId:'hector-qwen397',requestedModel:status.model,effectiveModel:out.model,fallback,fallbackLevel,reason,qwenConfigured,kimiConfigured,kimiModel:kimiStatus(c.env).model,contractApplied:contract.applied,contractReasons:contract.reasons}))
  ]);
  await saveRollingSummary(c.env.DB,userId,conversationId);
  return c.json({conversationId,message:{id:assistantMessageId,role:'assistant',content:contract.text},provider,model:out.model,requestedModel:status.model,runtimeId:'hector-qwen397',runtime:status,fallback,fallbackLevel,fallbackReason:fallback?reason:undefined,modelTier:effectiveQwen?'open-multimodal-moe-397b':'qwen397-fallback',usage:out.usage,contract:{applied:contract.applied,reasons:contract.reasons}});
