...
workEvidence.get('/jobs/:jobId',async c=>{
 ...
 const result=await c.env.DB.prepare(`SELECT e.id,e.source,e.url,e.title,e.http_status,e.errors_json,e.run_id,e.created_at,e.screenshot_key,f.id file_id
 ...
 const rows=(result.results??[]) as WorkEvidenceRow[];
