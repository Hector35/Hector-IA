...
import {KNOWLEDGE_VERSION,publicSelfModel} from '../intelligence/knowledge';
...
function checks(){return[
 {name:'identidad',prompt:'Explica en máximo 6 líneas quién eres, quién es tu propietario, cuál es tu arquitectura, qué modelo usas y cuáles son tus límites.',max:20,test:(t:string)=>/Héctor OS/i.test(t)&&/Héctor/i.test(t)&&/(Cloudflare|D1|R2|modelo)/i.test(t)&&/(límite|depend|no puedo|externo)/i.test(t)},
 ...
 {name:'ecosistema OpenAI',prompt:'Diferencia con precisión Héctor OS, ChatGPT Work, Codex y GPT-5.6. Indica el papel de cada uno y cuál conserva identidad y memoria del proyecto.',max:20,test:(t:string)=>/Héctor OS/i.test(t)&&/Work/i.test(t)&&/Codex/i.test(t)&&/GPT-5\.6/i.test(t)&&/(identidad|memoria|sistema completo)/i.test(t)}
 ];}

async function runSelfEvaluation(env:Bindings,db:D1Database,userId:string){
 const pack=await loadContextPack(env,userId,undefined,'autoevaluación verificable de inteligencia, memoria, identidad, arquitectura, OpenAI, Work, Codex, GPT-5.6 y funciones');
 const context=renderContext(pack),results:any[]=[];
 let rawTotal=0,totalLatency=0,lastModel='';
 const suite=checks(),rawMaximum=suite.reduce((sum,item)=>sum+item.max,0);
 for(const item of suite){
  const start=Date.now();
  try{
   const out=await respondContextual(env,item.prompt,[],context,false,{reasoning:'max'}),latency=Date.now()-start,passed=item.test(out.text),score=passed?item.max:0;
   rawTotal+=score;totalLatency+=latency;lastModel=out.model;
   results.push({name:item.name,score,max:item.max,passed,latencyMs:latency,response:out.text.slice(0,1600),model:out.model,reasoningEffort:out.reasoningEffort});
 ...
 const total=Math.round(rawTotal/rawMaximum*100),strengths=results.filter(x=>x.passed).map(x=>x.name),gaps=results.filter(x=>!x.passed).map(x=>x.name);
 ...
 const prompt=`Trabaja en el repositorio Hector35/Hector-IA. Héctor OS obtuvo ${total}/100 (${grade}) en su autoevaluación verificable usando ${lastModel||'modelo desconocido'}.\n\nFortalezas verificadas: ${strengths.join(', ')||'ninguna'}.\nPruebas fallidas: ${gaps.join(', ')||'ninguna'}.\n\nEvidencia de fallos:\n${JSON.stringify(failedEvidence,null,2)}\n\nAudita el código y corrige causas raíz. Cada conclusión debe vincularse a archivos, rutas, tablas, workflows o resultados reales. Ejecuta typecheck, tests, build, migraciones D1, despliegue y smoke tests. No declares éxito sin evidencia.`;
 const promptId=crypto.randomUUID(),averageLatencyMs=Math.round(totalLatency/suite.length);
 await db.batch([
  db.prepare('INSERT INTO self_evaluations(id,user_id,score,grade,strengths_json,gaps_json,tests_json,model,latency_ms) VALUES(?,?,?,?,?,?,?,?,?)').bind(evaluationId,userId,total,grade,JSON.stringify(strengths),JSON.stringify(gaps),JSON.stringify(results),lastModel,averageLatencyMs),
  db.prepare('INSERT INTO improvement_prompts(id,evaluation_id,user_id,target,prompt) VALUES(?,?,?,?,?)').bind(promptId,evaluationId,userId,'ChatGPT Work + Codex + GitHub',prompt)
 ]);
 return{evaluationId,promptId,score:total,rawScore:rawTotal,rawMaximum,grade,strengths,gaps,tests:results,averageLatencyMs,prompt,lastModel,knowledgeVersion:KNOWLEDGE_VERSION};
}
...
intelligence.post('/chat',async c=>{
 ...
 if(isSelfAnalysisRequest(message)){
  const report=await runSelfEvaluation(c.env,c.env.DB,userId),text=renderEvidenceSelfAnalysis(report);
  ...
  return c.json({conversationId:cid,message:{role:'assistant',content:text},provider:'system',model:`self-evaluation:${report.lastModel||'unknown'}`,modelTier:'verified',modelReason:'manifiesto + D1 + suite reproducible + GPT-5.6 Sol max',fallback:false,selfEvaluation:report});
 }
 if(/genera(?:me)? (?:el |un )?prompt de mejora|prompt para (?:chatgpt|github|codex|work)/i.test(message)){
 ...
 try{
  const out=await respondContextual(c.env,message,pack.recentMessages,context,config.allowWeb),usage=estimateCost(out.usage,out.model);
  ...
  await c.env.DB.batch([
   ...
   c.env.DB.prepare('INSERT INTO api_usage(id,user_id,provider,service,model,input_units,cached_input_units,output_units,estimated_cost_usd,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(crypto.randomUUID(),userId,provider,out.searchedWeb?'contextual-chat+web':'contextual-chat',out.model,usage.input,usage.cached,usage.output,usage.costUsd,JSON.stringify({tier:out.modelTier,reason:out.modelReason,reasoningEffort:out.reasoningEffort,task:out.task,searchedWeb:out.searchedWeb,providerReason:out.providerReason,fallback:out.fallback,knowledgeVersion:KNOWLEDGE_VERSION}))
  ]);
  if(config.allowLearning)await maybeSaveExplicitMemory(c.env,userId,message);
  await saveRollingSummary(c.env.DB,userId,cid);
  return c.json({conversationId:cid,message:{role:'assistant',content:out.text},usage,searchedWeb:out.searchedWeb,model:out.model,provider:out.provider,providerReason:out.providerReason,fallback:out.fallback,modelTier:out.modelTier,modelReason:out.modelReason,reasoningEffort:out.reasoningEffort,knowledgeVersion:KNOWLEDGE_VERSION,context:{memories:pack.memories.length,recentMessages:pack.recentMessages.length,hasSummary:!!pack.summary}});
 }catch(error){return c.json({error:error instanceof Error?error.message:'Error de IA contextual'},502);}
});

intelligence.get('/self-model',async c=>{
 ...
 const model=publicSelfModel({fast:c.env.OPENAI_MODEL_FAST||c.env.OPENAI_MODEL,balanced:c.env.OPENAI_MODEL_BALANCED||c.env.OPENAI_MODEL,reasoning:c.env.OPENAI_MODEL_REASONING||c.env.OPENAI_MODEL});
 return c.json({...model,runtime:{memories:Number(memory?.count||0),workJobs:{total:Number(work?.total||0),completed:Number(work?.completed||0),blocked:Number(work?.blocked||0)},scheduledTasks:{total:Number(schedules?.total||0),active:Number(schedules?.active||0)},activeSystemContexts:Number(contexts?.count||0),latestEvaluation:evaluation||null}});
});
...
intelligence.get('/status',async c=>{
 ...
 return c.json({evaluation,prompt,systemContext:contexts.results,knowledgeVersion:KNOWLEDGE_VERSION,models:{fast:c.env.OPENAI_MODEL_FAST||c.env.OPENAI_MODEL,balanced:c.env.OPENAI_MODEL_BALANCED||c.env.OPENAI_MODEL,reasoning:c.env.OPENAI_MODEL_REASONING||c.env.OPENAI_MODEL}});
});
