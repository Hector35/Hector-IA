...
async function startApprovalStatus(env:Bindings,userId:string,goalId:string){
 const row=await env.DB.prepare("SELECT status FROM hector_agent_approvals WHERE user_id=? AND goal_id=? AND action='start_goal' ORDER BY created_at DESC LIMIT 1")
  .bind(userId,goalId).first<{status:string}>();
...
hectorAgent.patch('/settings',async c=>{
 ...
 const updated=await settings(c.env,userId);
 if(updated.paused||!updated.auto_enabled){
...
hectorAgent.post('/goals',async c=>{
 ...
 const prompt=[
  ...
  'MEMORIA PERSISTENTE DISPONIBLE',persistentMemory,
...
hectorAgent.post('/goals/:id/pause',async c=>{
 const row=await c.env.DB.prepare('SELECT g.id,g.work_job_id,w.status FROM hector_agent_goals g JOIN work_jobs w ON w.id=g.work_job_id WHERE g.id=? AND g.user_id=?')
  .bind(c.req.param('id'),c.get('userId')).first<any>();if(!row)return c.json({error:'Objetivo no encontrado'},404);
...
hectorAgent.post('/goals/:id/run-now',async c=>{
 ...
 await c.env.DB.prepare("INSERT INTO work_events(id,job_id,message,progress) SELECT ?,id,'Ejecución priorizada manualmente; el cron la recogerá en menos de un minuto',progress FROM work_jobs WHERE id=?")
  .bind(crypto.randomUUID(),row.work_job_id).run();
...
hectorAgent.post('/approvals/:id/reject',async c=>{
 const userId=c.get('userId'),approval=await c.env.DB.prepare("SELECT id,goal_id,action FROM hector_agent_approvals WHERE id=? AND user_id=? AND status='pending'")
  .bind(c.req.param('id'),userId).first<any>();if(!approval)return c.json({error:'Aprobación no encontrada o ya resuelta'},404);
...
hectorAgent.post('/memory',async c=>{
 const parsed=memorySchema.safeParse(await c.req.json());if(!parsed.success)return c.json({error:'Memoria inválida'},400);
 const id=crypto.randomUUID();await c.env.DB.prepare('INSERT INTO hector_agent_memory(id,user_id,kind,content) VALUES(?,?,?,?)').bind(id,c.get('userId'),parsed.data.kind,parsed.data.content).run();
 return c.json({id,...parsed.data},201);
});

hectorAgent.patch('/memory/:id',async c=>{
 const parsed=memorySchema.partial().safeParse(await c.req.json());if(!parsed.success||(!parsed.data.kind&&!parsed.data.content))return c.json({error:'Memoria inválida'},400);
 const current=await c.env.DB.prepare('SELECT * FROM hector_agent_memory WHERE id=? AND user_id=?').bind(c.req.param('id'),c.get('userId')).first<any>();if(!current)return c.json({error:'Memoria no encontrada'},404);
 await c.env.DB.prepare('UPDATE hector_agent_memory SET kind=?,content=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?')
  .bind(parsed.data.kind??current.kind,parsed.data.content??current.content,current.id,c.get('userId')).run();
 return c.json({ok:true});
});

hectorAgent.delete('/memory/:id',async c=>{
 const result=await c.env.DB.prepare('DELETE FROM hector_agent_memory WHERE id=? AND user_id=?').bind(c.req.param('id'),c.get('userId')).run();
 if(!result.meta.changes)return c.json({error:'Memoria no encontrada'},404);
 return c.json({ok:true});
});
