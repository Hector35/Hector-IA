...
const file=z.object({path:z.string(),content:z.string().max(90000)});
const proposalRequest=z.object({jobId:z.string().uuid(),task:z.string().min(10).max(10000),attempt:z.number().int().min(1).max(3),failure:z.string().max(12000).optional(),files:z.array(file).min(1).max(20)});
const proposal=z.object({summary:z.string().min(1).max(3000),risk:z.literal('low'),hypothesis:z.string().max(3000).optional(),acceptance:z.array(z.string().max(500)).max(20).optional(),changes:z.array(file).max(6)});
...
async function enforceRunnerControl(env:Bindings,jobId:string){
 const control=await runnerControl(env,jobId);
 if(control.reason&&control.isHectorAgent&&control.status!=='blocked')await blockRunnerHectorGoal(env,jobId,control.reason);
 return control;
}

async function beginRunnerCycle(env:Bindings,jobId:string):Promise<RunnerControl>{
 const control=await enforceRunnerControl(env,jobId);if(control.reason||!control.isHectorAgent)return control;
