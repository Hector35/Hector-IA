...
export function buildSelfModel(models:{fast:string;balanced:string;reasoning:string},runtime:SelfModelRuntime){
 return{
  ...
  runtime,
  limitations:[
   'Héctor OS no posee ni modifica los pesos de los modelos externos.',
   'No tiene acceso a cuentas, dispositivos o servicios que no estén conectados y autorizados.',
   'No fusiona ni despliega cambios de código automáticamente.',
   'Los datos actuales de OpenAI, Work, Codex y GPT-5.6 requieren revalidación oficial.',
   'La configuración de un modelo no demuestra por sí sola el modelo efectivo de una ejecución.'
  ]
 };
}
...
selfModel.get('/',async c=>{
 ...
 const runtime:SelfModelRuntime={
  memories:Number(memory?.count||0),
  workJobs:{total:Number(work?.total||0),completed:Number(work?.completed||0),blocked:Number(work?.blocked||0)},
  scheduledTasks:{total:Number(schedules?.total||0),active:Number(schedules?.active||0)},
  activeSystemContexts:Number(contexts?.count||0),
