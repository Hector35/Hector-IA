...
import {applyVerifiedQualityEvidence} from '../intelligence/verified-quality-evidence';
...
const qualityReport=(metrics:QualityMetrics)=>applyVerifiedQualityEvidence(buildSystemQualityReport(metrics));

systemInfo.get('/stage-6',c=>c.json(stageSixStatus(c.env)));
systemInfo.get('/quality',async c=>c.json(qualityReport(await loadQualityMetrics(c.env.DB,c.get('userId')))));
...
systemInfo.get('/report',async c=>{
 ...
 const [latest,promptRow,quality]=await Promise.all([
  ...
  loadQualityMetrics(c.env.DB,userId).then(qualityReport)
 ]);
