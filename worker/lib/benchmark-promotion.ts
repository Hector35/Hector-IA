...
export type BenchmarkPolicy={strategy:'baseline'|'adaptive';status:'observing'|'promoted'|'rolled_back';sampleCount:number;adaptiveWinRate:number;averageScoreDelta:number;averageCostMultiplier:number;reason:string};
...
export async function loadBenchmarkPolicy(db:D1Database,userId:string,category:string):Promise<BenchmarkPolicy|null>{
 try{const row=await db.prepare('SELECT strategy,status,sample_count,adaptive_win_rate,average_score_delta,average_cost_multiplier,reason FROM intelligence_strategy_policies WHERE user_id=? AND category=?').bind(userId,category).first<any>();if(!row)return null;return{strategy:row.strategy,status:row.status,sampleCount:Number(row.sample_count),adaptiveWinRate:Number(row.adaptive_win_rate),averageScoreDelta:Number(row.average_score_delta),averageCostMultiplier:Number(row.average_cost_multiplier),reason:String(row.reason)};}catch{return null;}
}

export async function recomputeBenchmarkPolicy(db:D1Database,userId:string,category:string){
 const existing=await loadBenchmarkPolicy(db,userId,category),rows=(await db.prepare(`SELECT winner,baseline_score,adaptive_score,baseline_cost_usd,adaptive_cost_usd,created_at FROM intelligence_benchmark_results WHERE user_id=? AND category=? AND created_at>=datetime('now',?) ORDER BY created_at DESC LIMIT ?`).bind(userId,category,`-${BENCHMARK_PROMOTION_POLICY.windowDays} days`,BENCHMARK_PROMOTION_POLICY.sampleLimit).all<BenchmarkAggregateRow>()).results||[],policy=summarizeBenchmarkPolicy(rows,existing||undefined),nowPromoted=policy.status==='promoted'&&!existing?.status?.includes('promoted'),nowRolledBack=policy.status==='rolled_back'&&existing?.status==='promoted';
