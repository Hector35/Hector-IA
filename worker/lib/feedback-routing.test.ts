...
describe('summarizeFeedbackRouting',()=>{
 ...
 it('nunca considera las respuestas profundas para exigir más profundidad',()=>{const profile=summarizeFeedbackRouting(Array.from({length:8},()=>row(-1,'openai','deep')));expect(profile.nonDeepSampleCount).toBe(0);expect(profile.preferDeep).toBe(false);});
});
