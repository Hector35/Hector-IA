...
const policy=(state:BudgetQualityPolicy['state']='healthy'):BudgetQualityPolicy=>({state,sampleCount:8,acceptedCount:7,negativeCount:1,correctionCount:0,acceptanceRate:.875,negativeRate:.125,correctionRate:0,lastDegradedAt:null,nextProbeAt:null,triggeredBy:[],reason:'calidad estable'});
describe('budget quality governance',()=>{
 ...
 it('desactiva ahorro permanentemente en modo proteger',()=>expect(applyBudgetQualityGovernance(policy(),{mode:'protect',reason:'Salud requiere máxima calidad',updatedAt:null}).state).toBe('suspended'));
 it('limita la categoría a pruebas controladas',()=>expect(applyBudgetQualityGovernance(policy(),{mode:'probe',reason:'Validar antes de ahorrar',updatedAt:null}).state).toBe('probe'));
