...
export function chooseProvider(input:string,tier:'fast'|'balanced'|'deep',needsWeb:boolean,enabled=true,health?:ProviderHealth,feedback?:FeedbackRoutingProfile):ProviderDecision{
 ...
 if(deterministic)return{provider:'cloudflare',reason:`operación ${deterministic.capability} resuelta con reglas locales sin inferencia`};
 ...
 if(capability!=='general')return{provider:'cloudflare',reason:`capacidad ligera ${capability} asignada al escalón gratuito`};
...
export async function callCloudflare(env:Bindings,instructions:string,input:string){
 const deterministic=tryDeterministicFreeInference(input);
 if(deterministic)return{text:deterministic.text,id:`rules-${crypto.randomUUID()}`,model:'hector-rules-v1',usage:{input_tokens:0,output_tokens:0},capability:deterministic.capability};
