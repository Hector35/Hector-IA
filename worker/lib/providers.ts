...
export function chooseProvider(input:string,tier:'fast'|'balanced'|'deep',needsWeb:boolean,enabled=true,health?:ProviderHealth,feedback?:FeedbackRoutingProfile,qwenConfigured=true):ProviderDecision{
 ...
 if(explicitQwenRequest.test(input))return qwenConfigured?{provider:'huggingface',reason:'el usuario solicitó explícitamente Héctor Qwen'}:{provider:'cloudflare',reason:'Héctor Qwen no está habilitado; se usa Workers AI sin OpenAI'};
 if(qwenConfigured){
 ...
 if(!enabled)return{provider:'cloudflare',reason:'Qwen no está habilitado y Workers AI está desactivado; OpenAI no está autorizado en el chat'};
...
function qwenMessages(input:unknown):Array<{role:'system'|'user'|'assistant';content:string}>{if(Array.isArray(input))return input.filter((turn):turn is QwenTurn=>Boolean(turn)&&typeof turn==='object'&&((turn as QwenTurn).role==='user'||(turn as QwenTurn).role==='assistant')&&typeof (turn as QwenTurn).content==='string').map((turn,index,items)=>({role:turn.role,content:turn.role==='user'&&index===items.length-1?stripQwenDirective(turn.content):turn.content}));return[{role:'user',content:stripQwenDirective(String(input??''))}];}
