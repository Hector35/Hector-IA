...
export const CHAT_RUNTIME_POLICY={provider:'cloudflare',openAIAllowed:false,openAIRole:'training-and-evaluation-only'} as const;
...
export function chooseProvider(input:string,tier:'fast'|'balanced'|'deep',needsWeb:boolean,enabled=true,health?:ProviderHealth,feedback?:FeedbackRoutingProfile):ProviderDecision{
 ...
 if(!enabled)return{provider:'cloudflare',reason:'OpenAI está bloqueado en el chat y Workers AI está desactivado; la ejecución debe fallar de forma visible'};
 ...
 return{provider:'cloudflare',reason:health?.reason==='enfriamiento completado; se permite una prueba controlada'?'prueba controlada de Héctor Base tras enfriamiento':'Héctor Base es el único runtime conversacional autorizado'};
}
