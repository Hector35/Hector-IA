...
const task:ScheduledTaskDefinition={
 id:'11111111-1111-4111-8111-111111111111',title:'Continuar Héctor OS',prompt:'Revisa el estado actual y realiza la siguiente mejora prioritaria.',kind:'programming',cadence:'hourly',interval_minutes:60,reasoning_level:'high',autonomy_mode:'continuous',allow_web:1,next_run_at:'2026-07-21T12:00:00.000Z'
};

describe('scheduler',()=>{
 it('convierte cadencias e intervalos personalizados de forma controlada',()=>{expect(intervalMinutesFor('once')).toBeNull();expect(intervalMinutesFor('hourly')).toBe(60);expect(intervalMinutesFor('hourly',5)).toBe(5);expect(intervalMinutesFor('hourly',0)).toBe(1);expect(intervalMinutesFor('weekly',20000)).toBe(10080);});
 it('salta intervalos vencidos sin crear una tormenta de ejecuciones',()=>{expect(nextScheduledRun('hourly','2026-07-21T12:00:00.000Z',Date.parse('2026-07-21T15:40:00.000Z'))).toBe('2026-07-21T16:00:00.000Z');expect(nextScheduledRun('hourly','2026-07-21T12:00:00.000Z',Date.parse('2026-07-21T12:12:00.000Z'),5)).toBe('2026-07-21T12:15:00.000Z');expect(nextScheduledRun('once','2026-07-21T12:00:00.000Z')).toBeNull();});
