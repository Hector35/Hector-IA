...
export type PolicyEvidence={
 source:'base'|'explicit'|'benchmark'|'feedback'|'budget';
 outcome:'applied'|'ignored'|'protected';
 reason:string;
};
export type PolicyDecision={
 ...
 budgetDecision:ReturnType<typeof summarizeBudgetDecision>|null;
...
export function resolvePreBudgetPolicy(input:{baseRoute:PolicyRoute;models:PolicyModels;options:PolicyOptions;benchmark:BenchmarkPolicy|null;feedback:FeedbackRoutingProfile}):PolicyDecision{
 ...
 if(options.reasoning==='high'){
  route=deepRoute(route,input.models,'nivel de razonamiento alto solicitado explícitamente');
  evidence.push({source:'explicit',outcome:'applied',reason:'el propietario solicitó razonamiento alto'});
 }
 const promoted=input.benchmark?.strategy==='adaptive'&&input.benchmark.status==='promoted';
 if(promoted&&!manualControl(input.options)){
  route=deepRoute(route,input.models,`promoción por benchmark: ${input.benchmark?.reason||'estrategia adaptativa validada'}`);
  options={...options,reasoning:'high',deliberation:'force'};
  benchmarkApplied=true;
  evidence.push({source:'benchmark',outcome:'applied',reason:input.benchmark?.reason||'estrategia adaptativa promovida'});
 }else if(promoted){
  evidence.push({source:'benchmark',outcome:'ignored',reason:'el control manual del propietario tiene precedencia'});
 }
 if(input.feedback.preferDeep&&route.tier!=='deep'){
  route=deepRoute(route,input.models,`aprendizaje del libro mayor: ${input.feedback.reason}`);
  feedbackApplied=true;
  evidence.push({source:'feedback',outcome:'applied',reason:input.feedback.reason});
 }else if(input.feedback.preferDeep){
  evidence.push({source:'feedback',outcome:'ignored',reason:'la ruta ya usa razonamiento profundo'});
 }
...
export function resolveBudgetPolicy(input:{decision:PolicyDecision;models:PolicyModels;budget:CognitiveBudgetStatus;projection:CognitiveCostProjection|null;inputText:string}):PolicyDecision{
 const explicitHigh=input.decision.options.reasoning==='high'||input.decision.options.deliberation==='force';
 const sensitive=isBudgetProtectedTask(input.inputText,input.decision.route.task);
 const action=budgetAction(input.budget,sensitive,explicitHigh,input.projection);
 const evidence=[...input.decision.evidence];let route=input.decision.route;
 if(action.action==='degrade'){
  route=downgrade(route,input.models,'protección presupuestaria predictiva');
  evidence.push({source:'budget',outcome:'applied',reason:action.reason});
 }else if(action.action==='allow-protected'){
  evidence.push({source:'budget',outcome:'protected',reason:action.reason});
 }else{
  evidence.push({source:'budget',outcome:'ignored',reason:action.reason});
 }
...
export function explainPolicy(evidence:PolicyEvidence[],route:PolicyRoute){
 const applied=evidence.filter(item=>item.outcome==='applied').map(item=>item.reason);
 const protectedItems=evidence.filter(item=>item.outcome==='protected').map(item=>item.reason);
 const ignored=evidence.filter(item=>item.outcome==='ignored').map(item=>item.reason);
 return[`Ruta final ${route.tier} (${route.model}).`,applied.length?`Aplicado: ${applied.join(' → ')}.`:'',protectedItems.length?`Protegido: ${protectedItems.join(' · ')}.`:'',ignored.length?`Ignorado por precedencia: ${ignored.join(' · ')}.`:''].filter(Boolean).join(' ');
}
