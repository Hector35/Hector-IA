...
export type FreeFirstDecision={capability:FreeFirstCapability;tier:FreeFirstTier;risk:FreeFirstRisk;reason:string;maxAdvancedCalls:0|1;timeoutMs:number};
export type DeterministicFreeResult={text:string;capability:'classify'|'extract'|'route';confidence:number};
...
export function decideFreeFirst(prompt:string,options:{allowWeb?:boolean;reasoningHigh?:boolean;freeProviderHealthy?:boolean}={}):FreeFirstDecision{
 const capability=detectFreeFirstCapability(prompt),risk:FreeFirstRisk=highRisk.test(prompt)?'high':complexSignals.test(prompt)?'medium':'low';
...
function taskCategory(text:string){const value=normalized(text);if(/github|repositorio|codigo|typescript|build|test/.test(value))return'github-code';if(/calendario|reunion|evento|agenda/.test(value))return'calendar';if(/correo|email|gmail|mensaje/.test(value))return'communication';if(/documento|pdf|archivo|drive/.test(value))return'documents';if(/dinero|saldo|gasto|presupuesto/.test(value))return'finance';return'general';}
...
export function runDeterministicFreeInference(prompt:string):DeterministicFreeResult|null{
 ...
 if(capability==='classify'){const category=taskCategory(requestedBody(prompt));return{text:JSON.stringify({category,source:'deterministic',advancedCalls:0}),capability,confidence:.9};}
 if(capability==='route'){const taskType=taskCategory(requestedBody(prompt));return{text:JSON.stringify({taskType,recommendedPath:taskType,source:'deterministic',advancedCalls:0}),capability,confidence:.88};}
 if(capability==='extract'){
  const target=requestedBody(prompt),emails=[...new Set(target.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi)||[])],urls=[...new Set(target.match(/https?:\/\/[^\s)]+/gi)||[])],amounts=[...new Set(target.match(/(?:\$|MXN\s*)\s?\d[\d,]*(?:\.\d{1,2})?/gi)||[])];
  return{text:JSON.stringify({emails,urls,amounts,source:'deterministic',advancedCalls:0}),capability,confidence:.95};
 }
 return null;
}

export async function withTimeout<T>(operation:()=>Promise<T>,timeoutMs:number):Promise<T>{let timer:ReturnType<typeof setTimeout>|undefined;try{return await Promise.race([operation(),new Promise<T>((_,reject)=>{timer=setTimeout(()=>reject(new Error(`free-first timeout after ${timeoutMs}ms`)),timeoutMs);})]);}finally{if(timer)clearTimeout(timer);}}
