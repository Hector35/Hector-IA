...
describe('evaluateContainmentProbe',()=>{
 it('mantiene la prueba acotada mientras reúne evidencia',()=>{const probe=evaluateContainmentProbe([row('baseline',13,14,.01,.02),row('tie',14,14,.01,.02)],[containment('probe-ready')],new Date('2026-07-22T06:00:00Z').getTime());expect(probe.state).toBe('running');expect(probe.sampleCount).toBe(2);expect(probe.maxSamples).toBe(8);});
 it('hace rollback temprano ante pérdida severa',()=>{const probe=evaluateContainmentProbe([row('adaptive',8,14,.01,.02),row('adaptive',8,14,.01,.02)],[containment('probe-ready')]);expect(probe.state).toBe('failed');expect(probe.reason).toContain('rollback inmediato');});
 it('aprueba baseline cuando ahorra al menos 20% sin pérdida material',()=>{const rows=Array.from({length:5},(_,index)=>row(index<3?'baseline':'tie',13.5,14,.01,.02,`2026-07-22T0${5+index}:00:00.000Z`));const probe=evaluateContainmentProbe(rows,[containment('probe-ready')]);expect(probe.state).toBe('passed');expect(probe.baselineWinRate).toBeGreaterThanOrEqual(.6);expect(probe.averageCostRatio).toBeLessThanOrEqual(.8);});
 ...
 it('ignora muestras anteriores al inicio de la prueba',()=>{const probe=evaluateContainmentProbe([row('baseline',15,14,.01,.02,'2026-07-22T03:00:00.000Z'),row('tie',14,14,.01,.02,'2026-07-22T05:00:00.000Z')],[containment('probe-ready')]);expect(probe.sampleCount).toBe(1);});
});
...
describe('applyDriftContainmentProtection',()=>{
 ...
 it('reactiva protección cuando la prueba pierde calidad',()=>{const rows=[row('adaptive',8,14,.01,.02),row('adaptive',8,14,.01,.02)];const policy=applyDriftContainmentProtection(base,[containment('probe-ready')],'auto',rows);expect(policy.strategy).toBe('adaptive');expect(policy.containmentProbe?.state).toBe('failed');});
 it('mantiene baseline tras una prueba aprobada',()=>{const rows=Array.from({length:5},(_,index)=>row(index<3?'baseline':'tie',13.5,14,.01,.02,`2026-07-22T0${5+index}:00:00.000Z`));const policy=applyDriftContainmentProtection(base,[containment('probe-ready')],'auto',rows);expect(policy.strategy).toBe('baseline');expect(policy.status).toBe('containment_probe_passed');});
