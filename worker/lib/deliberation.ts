...
function compactText(value:string,maxChars:number){
 ...
 const marker='\n[…contenido intermedio compactado…]\n',remaining=maxChars-marker.length,head=Math.ceil(remaining*.72),tail=Math.max(0,remaining-head);
...
export function judgeInput(original:string,candidates:Array<{role:CandidateRole;text:string}>){
 ...
 const available=Math.max(JUDGE_MIN_CANDIDATE_CHARS*selected.length,JUDGE_MAX_CHARS-fixedChars),perCandidate=selected.length?Math.max(JUDGE_MIN_CANDIDATE_CHARS,Math.floor(available/selected.length)):0;
 ...
 const overflow=assembled.length-JUDGE_MAX_CHARS,saferBudget=Math.max(JUDGE_MIN_CANDIDATE_CHARS,perCandidate-Math.ceil(overflow/Math.max(selected.length,1)));
...
export function aggregateUsage(usages:Array<UsageLike|undefined>):UsageLike{
 const input=usages.reduce((sum,item)=>sum+(item?.input_tokens||0),0),output=usages.reduce((sum,item)=>sum+(item?.output_tokens||0),0),cached=usages.reduce((sum,item)=>sum+(item?.input_tokens_details?.cached_tokens||0),0);
