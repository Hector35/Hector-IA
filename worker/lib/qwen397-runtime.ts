...
export const QWEN_397_PROBE_TOKEN='HECTOR_QWEN397_OK';
...
export function verifyQwen397ProbeText(value:unknown){
 const text=String(value||'').trim();
 if(text!==QWEN_397_PROBE_TOKEN)throw new Error(`La prueba de Qwen 397B devolvió contenido inesperado: ${text.slice(0,120)||'(vacío)'}`);
 return text;
}
...
type QwenRequestOptions={maxTokens:number;temperature:number;topP:number};

async function requestQwen397(env:Bindings,messages:QwenMessage[],options:QwenRequestOptions){
 ...
 try{
  const response=await fetch(chatEndpoint(endpoint.baseUrl),{
   ...
   body:JSON.stringify({model:endpoint.requestModel,messages,max_tokens:options.maxTokens,temperature:options.temperature,top_p:options.topP,stream:false}),
  ...
  return{text,reasoningContent:data.choices?.[0]?.message?.reasoning_content,id:data.id||`qwen397-${crypto.randomUUID()}`,model:effectiveModel,requestedModel:endpoint.requestedModel,endpointSource:endpoint.source,selectionPolicy:endpoint.selectionPolicy,billingMode:endpoint.billingMode,latencyMs:Date.now()-started,usage:{input_tokens:data.usage?.prompt_tokens,output_tokens:data.usage?.completion_tokens},runtime:QWEN_397_OPERATIONAL};
 }catch(error){
...
export function callQwen397(env:Bindings,instructions:string,messages:Array<{role:'user'|'assistant';content:string}>){return requestQwen397(env,[{role:'system',content:instructions},...messages],{maxTokens:4096,temperature:.6,topP:.95});}
export function callQwen397Vision(env:Bindings,prompt:string,dataUrl:string){return requestQwen397(env,[{role:'system',content:'Analiza imágenes con precisión. Separa observaciones, inferencias y límites. Responde en español salvo petición contraria.'},{role:'user',content:[{type:'text',text:prompt},{type:'image_url',image_url:{url:dataUrl}}]}],{maxTokens:2048,temperature:.4,topP:.9});}
export async function probeQwen397(env:Bindings){
 const out=await requestQwen397(env,[{role:'system',content:`Prueba de atribución. Devuelve exactamente ${QWEN_397_PROBE_TOKEN}, sin puntuación, explicación ni formato adicional.`},{role:'user',content:'Ejecuta la prueba de atribución exacta.'}],{maxTokens:24,temperature:0,topP:1});
 verifyQwen397ProbeText(out.text);
 return{...out,attested:true,probeToken:QWEN_397_PROBE_TOKEN};
}
