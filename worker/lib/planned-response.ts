...
export async function executePlannedContextual(env:Bindings,input:string,history:PlannedChatTurn[],renderedContext:string,plan:ExecutionPlan,metadata:PlannedExecutionMetadata){const started=Date.now(),system=instructions(plan,renderedContext,metadata.feedback.guidance),chatInput=conversation(history,input),result=plan.cognition.mode==='ensemble'?await ensemble(env,chatInput,system,plan,started):await single(env,chatInput,system,plan,started);return decorate(result,plan,metadata);}
