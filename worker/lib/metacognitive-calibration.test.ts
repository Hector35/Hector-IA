...
describe('metacognitive calibration',()=>{
 ...
 it('detecta sobreconfianza repetida y escala conservadoramente',()=>{const profile=summarizeMetacognitiveCalibration(Array.from({length:10},()=>({predicted_confidence:.95,correct:0,route_tier:'fast',provider:'cloudflare'})));const route=applyMetacognitiveRouting({tier:'fast' as const,reasoning:'low' as const,reason:'ruta barata'},profile);expect(profile).toMatchObject({preferDeep:true,requireVerification:true});expect(route).toMatchObject({tier:'deep',reasoning:'high'});});
 it('preserva una ruta calibrada',()=>{const rows=Array.from({length:10},(_,index)=>({predicted_confidence:index<8?.8:.2,correct:index<8?1:0,route_tier:'fast',provider:'cloudflare'})),profile=summarizeMetacognitiveCalibration(rows),route=applyMetacognitiveRouting({tier:'fast' as const,reasoning:'low' as const,reason:'ruta barata'},profile);expect(profile.preferDeep).toBe(false);expect(route.tier).toBe('fast');});
