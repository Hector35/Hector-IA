...
describe('pricingForModel',()=>{
 it('reconoce las tres capas GPT-5.6 y snapshots',()=>{
  expect(pricingForModel('gpt-5.6-sol').outputPerMTok).toBe(20);
  expect(pricingForModel('gpt-5.6-terra-2026-07-21').inputPerMTok).toBe(2);
  expect(pricingForModel('gpt-5.6-luna').cachedInputPerMTok).toBe(.02);
...
describe('estimateModelCost',()=>{
 it('calcula Terra con lectura, escritura de caché y contexto largo',()=>{
  ...
  expect(result.costUsd).toBeCloseTo(5.18,6);
 ...
 it('aplica recargo de contexto largo a entrada y salida',()=>{
  ...
  expect(result.longContext).toBe(true);
  expect(result.costUsd).toBeCloseTo(.156,6);
 });
 it('no permite que los subconjuntos de caché excedan la entrada total',()=>{
  ...
  expect(result.costUsd).toBeCloseTo(.00004,8);
 });
});
