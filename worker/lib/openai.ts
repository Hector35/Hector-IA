...
export function routeModel(env:Bindings,input:string,allowWeb:boolean):Route{
 ...
 const selfKnowledge=selfKnowledgeSignals.test(input),productCurrent=selfKnowledge&&productFreshnessSignals.test(input),needsWeb=allowWeb&&(currentSignals.test(input)||productCurrent),hasDeepSignal=deepSignals.test(input),hasPlanningSignal=planningSignals.test(input),isTechnicalPlan=codeSignals.test(input)&&hasPlanningSignal,isComplexPlan=hasDeepSignal&&hasPlanningSignal;
...
function toolsFor(route:Route){return route.needsWeb?[{type:'web_search',search_context_size:route.tier==='deep'?'high':'medium'}]:undefined;}
...
async function executeOpenAIEnsemble(env:Bindings,input:string,instructions:string,route:Route,openAIInput:unknown,started:number,providerReason:string,profile:DeliberationProfile){
 ...
 const candidates=settled.flatMap(result=>result.status==='fulfilled'?[result.value]:[]);if(!candidates.length){const firstFailure=settled.find(result=>result.status==='rejected') as PromiseRejectedResult|undefined;throw firstFailure?.reason instanceof Error?firstFailure.reason:new Error('No se obtuvo ninguna solución candidata');}
