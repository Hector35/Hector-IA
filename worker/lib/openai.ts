...
export const CONTEXTUAL_ENGINE_VERSION='2.5.0';
...
export function applyUserFeedback(env:Bindings,route:Route,feedback:UserFeedbackProfile):Route{if(!feedback.preferDeep||route.tier==='deep')return route;return{...route,model:env.OPENAI_MODEL_REASONING||route.model,tier:'deep',reasoning:'high',reason:`aprendizaje controlado: ${feedback.reason}`};}
...
export async function respond(env:Bindings,input:string,previousResponseId:string|undefined,memories:string[],allowWeb=true,options:ResponseOptions={}){const base=applyResponseOptions(env,routeModel(env,input,allowWeb),options),feedback=await resolveFeedback(env,options.userId,base.task),route=applyUserFeedback(env,base,feedback),selectedMemories=relevantMemories(input,memories),instructions=legacyPrompt(route,selectedMemories,feedback.guidance);const result=await executeHybrid(env,input,instructions,route,input,feedback);return{...result,modelTier:route.tier,modelReason:route.reason,task:route.task,feedbackAdaptation:feedbackResult(feedback)};}
...
export async function respondContextual(env:Bindings,input:string,history:ChatTurn[],renderedContext:string,allowWeb=true,options:ResponseOptions={}){const base=applyResponseOptions(env,routeModel(env,input,allowWeb),options),feedback=await resolveFeedback(env,options.userId,base.task),route=applyUserFeedback(env,base,feedback);const instructions=`${renderBootstrap()}\n\n${behaviorRules(route,feedback.guidance)}\n\nCONTEXTO DINÁMICO\n${renderedContext}`;const conversation=conversationInput(history,input),result=await executeHybrid(env,input,instructions,route,conversation,feedback);return{...result,modelTier:route.tier,modelReason:route.reason,task:route.task,feedbackAdaptation:feedbackResult(feedback)};}
export async function inspectImage(env:Bindings,prompt:string,dataUrl:string){const model=env.OPENAI_MODEL_BALANCED||env.OPENAI_MODEL;const instructions=`${renderBootstrap()}\n\nAnaliza la imagen para Héctor. Responde en español, directo, preciso y útil. Separa observaciones visibles, inferencias y recomendaciones. No afirmes como visible algo que solo estás infiriendo.`;const out=await callResponses(env,{model,instructions,input:[{role:'user',content:[{type:'input_text',text:prompt},{type:'input_image',image_url:dataUrl,detail:'auto'}]}],store:false,reasoning:{effort:'medium'}});return{text:out.text,usage:out.data.usage,model,provider:'openai' as ProviderName};}
