...
export async function loadContextPack(env:Bindings,userId:string,conversationId:string|undefined,input:string):Promise<ContextPack>{
  const db=env.DB;
  const [systemRows,memoryRows,summaryRow,priorRows,recentRows,crossRows,workRows,updateRows,hubRows]=await Promise.all([
  ...
  const crossConversationMessages=relevantAndRecent(input,crossMessageText,8,4,10);
