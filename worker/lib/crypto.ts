...
// Workers Free allows 10 ms of CPU per request. 50k PBKDF2-SHA256 rounds
// stay below that budget while retaining a unique 128-bit salt per password.
export async function hashPassword(password: string, salt = randomToken(16)) { const key = await crypto.subtle.importKey('raw', encoder.encode(password), 'PBKDF2', false, ['deriveBits']); const bits = await crypto.subtle.deriveBits({ name:'PBKDF2', salt:encoder.encode(salt), iterations:50_000, hash:'SHA-256' }, key, 256); return { hash:hex(bits), salt }; }
