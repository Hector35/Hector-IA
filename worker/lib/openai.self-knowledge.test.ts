...
describe('self knowledge routing',()=>{
 it.each([
  ...
  'Háblame de GPT-5.6',
  '¿Qué sabes sobre mí?'
 ])('usa GPT-5.6 Sol con inteligencia profunda para %s',(input)=>{
  const route=routeModel(env,input,true);
  expect(route.tier).toBe('deep');
  expect(route.reasoning).toBe('xhigh');
 ...
 it('no degrada consultas normales al modelo profundo',()=>{
  ...
  expect(routeModel(env,'hola',true)).toMatchObject({tier:'fast',model:'gpt-5.6-luna',reasoning:'low'});
 });
});
