...
describe('self knowledge routing',()=>{
 ...
 it('activa web cuando la pregunta de producto depende del estado actual',()=>{expect(routeModel(env,'¿Cuál es la disponibilidad actual de GPT-5.6?',true).needsWeb).toBe(true);expect(routeModel(env,'¿Cuál es la disponibilidad actual de GPT-5.6?',false).needsWeb).toBe(false);});
 it('no degrada consultas normales al modelo profundo',()=>{expect(classify('hola')).toBe('consulta general');expect(routeModel(env,'hola',true)).toMatchObject({tier:'fast',model:'gpt-5.6-luna',reasoning:'low'});});
});
