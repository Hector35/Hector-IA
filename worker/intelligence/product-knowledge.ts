...
export const CAPABILITY_MATRIX=[
 ...
 {capability:'Inteligencia máxima',components:['gpt-5.6-sol','OPENAI_MODEL_REASONING','router profundo','reasoning effort max'],evidence:'modelo, tier y esfuerzo devueltos en respuesta y telemetría',limit:'la configuración no prueba por sí sola el modelo efectivo; debe observarse la ejecución'},
 ...
 {capability:'Control de calidad',components:['provider_quality_events','fallback','circuit breaker'],evidence:'puntaje, proveedor solicitado/efectivo y latencia registrados',limit:'una heurística de calidad no sustituye verificación factual'}
] as const;
...
export function renderProductKnowledge(){
 ...
 return `CONOCIMIENTO DE IDENTIDAD, PRODUCTOS Y CAPACIDADES\nVersión: ${PRODUCT_KNOWLEDGE_VERSION}. Verificado: ${PRODUCT_KNOWLEDGE_VERIFIED_AT}.\n${facts}\n\nMATRIZ DE CAPACIDADES VERIFICABLES\n${capabilities}\n\nREGLA DE ACTUALIDAD\nLos hechos marcados como time-sensitive sobre OpenAI, Work, Codex o GPT‑5.6 deben comprobarse únicamente con fuentes oficiales actuales cuando la pregunta dependa de disponibilidad, precios, planes, modelos, funciones o límites presentes.`;
}
