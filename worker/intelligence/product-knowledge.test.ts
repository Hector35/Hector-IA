...
describe('product knowledge',()=>{
 it('cubre identidad y separación de capas',()=>{expect(new Set(PRODUCT_FACTS.map(x=>x.subject))).toEqual(new Set(['hector-os','openai','work','codex','gpt-5.6','owner']));const rendered=renderProductKnowledge();for(const value of ['Héctor OS','Qwen3-30B','Qwen3-4B','OpenAI no está autorizado','entrenamiento',PRODUCT_KNOWLEDGE_VERIFIED_AT])expect(rendered).toContain(value);});
 it('distingue runtime, candidato y campeón',()=>{const rendered=renderProductKnowledge();for(const value of ['Qwen3 operativo','Utilidades abiertas','pesos abiertos base','no existe un campeón'])expect(rendered).toContain(value);});
