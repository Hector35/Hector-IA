...
import type {LearnedSkillSignal} from './learned-skills';
...
describe('learned skill lifecycle',()=>{
 ...
 it('promueve solo después de tres éxitos sin fallos',()=>{
  let signal:LearnedSkillSignal={successes:0,failures:0,confidence:.4,status:'candidate'};
 ...
 it('degrada, desactiva y no reactiva automáticamente una habilidad deshabilitada',()=>{
  let signal:LearnedSkillSignal=nextLearnedSkillSignal({successes:3,failures:0,confidence:.9,status:'active'},'failure');expect(signal.status).toBe('active');
