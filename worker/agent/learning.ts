...
export async function recordExperience(env:Bindings,input:RecordExperienceInput){
 ...
 await env.DB.prepare(`INSERT OR REPLACE INTO agent_experiences(
  ...
  provider,model,input_tokens,cached_input_tokens,output_tokens,estimated_cost_usd,created_at,updated_at
 ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,COALESCE((SELECT created_at FROM agent_experiences WHERE user_id=? AND execution_id=?),CURRENT_TIMESTAMP),CURRENT_TIMESTAMP)`).bind(
  ...
  finite(input.usage?.outputTokens),finite(input.usage?.estimatedCostUsd),input.userId,executionId
 ).run();
 return id;
}
