...
def args():
 p=argparse.ArgumentParser(); p.add_argument('--output-dir',required=True); p.add_argument('--train-file',required=True); p.add_argument('--validation-file',required=True); p.add_argument('--experiment-id',default='hector-asi-qwen15-v12'); p.add_argument('--epochs',type=int,default=3); p.add_argument('--max-length',type=int,default=112); p.add_argument('--lr',type=float,default=4e-5); p.add_argument('--min-lr',type=float,default=1e-5); p.add_argument('--seed',type=int,default=35); return p.parse_args()
def read(path):
...
def encode(tok,rows,n):
 out=[]
 for row in rows:
  b=tok(render(tok,row),return_tensors='pt',truncation=True,max_length=n); b['labels']=b['input_ids'].clone(); out.append(b)
 return out
...
def main():
 ...
 model=get_peft_model(base,cfg); pars=[p for p in model.parameters() if p.requires_grad]; before=torch.cat([p.detach().flatten().clone() for p in pars]); opt=AdamW(pars,lr=a.lr); sched=CosineAnnealingLR(opt,T_max=a.epochs*len(tb),eta_min=a.min_lr); losses=[]; lrs=[]; model.train()
 for _ in range(a.epochs):
  for b in tb:
   opt.zero_grad(set_to_none=True); z=model(**b).loss
   if not torch.isfinite(z): raise RuntimeError('non-finite loss')
   z.backward(); torch.nn.utils.clip_grad_norm_(pars,1.0); opt.step(); sched.step(); losses.append(float(z.detach().cpu())); lrs.append(float(opt.param_groups[0]['lr']))
