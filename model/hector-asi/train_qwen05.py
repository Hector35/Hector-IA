...
TRAIN='model/hector-asi/data/smollm360-sft-v1.jsonl'
VALIDATION='model/hector-asi/evals/smollm360-held-out-v1.jsonl'

def parse_args():
 p=argparse.ArgumentParser(); p.add_argument('--output-dir',default='artifacts/hector-asi-qwen05-v1'); p.add_argument('--epochs',type=int,default=1); p.add_argument('--max-length',type=int,default=80); p.add_argument('--lr',type=float,default=2e-4); p.add_argument('--seed',type=int,default=35); return p.parse_args()
...
def main():
 ...
 metrics={'schemaVersion':'1.0.0','experimentId':'hector-asi-qwen05-v1','status':'experimental-weights-generated','eligibleForChampion':False,'baseModel':{'repository':MODEL,'revision':REVISION,'license':'Apache-2.0'},'method':'LoRA SFT','dataset':{'trainPath':TRAIN,'trainSha256':sha(TRAIN),'trainExamples':len(tr),'validationPath':VALIDATION,'validationSha256':sha(VALIDATION),'validationExamples':len(va),'privateUserData':False},'hyperparameters':{'epochs':a.epochs,'maxLength':a.max_length,'learningRate':a.lr,'seed':a.seed,'loraRank':8,'loraAlpha':16,'loraDropout':0.0,'targetModules':['q_proj','v_proj']},'results':{'steps':len(losses),'firstTrainLoss':losses[0],'finalTrainLoss':losses[-1],'baseValidationLoss':base_loss,'candidateValidationLoss':cand_loss,'validationLossDelta':cand_loss-base_loss,'relativeValidationImprovement':(base_loss-cand_loss)/base_loss,'parameterDeltaL2':delta,'trainableParameters':sum(p.numel() for p in pars),'weightsGenerated':True,'adapterBytes':weights.stat().st_size,'adapterSha256':sha(weights)},'runtime':{'seconds':time.time()-started,'python':platform.python_version(),'torch':torch.__version__,'cpuCount':os.cpu_count(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}}
