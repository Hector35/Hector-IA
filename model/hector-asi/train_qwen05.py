...
def args():
 p=argparse.ArgumentParser(); p.add_argument('--output-dir',default='artifacts/hector-asi-qwen05-v1'); p.add_argument('--epochs',type=int,default=1); p.add_argument('--max-length',type=int,default=96); p.add_argument('--lr',type=float,default=2e-4); p.add_argument('--seed',type=int,default=35); return p.parse_args()
def read(path):
...
def encode(tok,rows,n):
 out=[]
 for r in rows:
  b=tok(render(tok,r),return_tensors='pt',truncation=True,max_length=n); b['labels']=b['input_ids'].clone(); out.append(b)
 return out
...
def main():
 ...
 trainp='model/hector-asi/data/qwen05-sft-v1.jsonl'; valp='model/hector-asi/evals/qwen05-held-out-v1.jsonl'; tr,va=read(trainp),read(valp)
 ...
 resolved=getattr(base.config,'_commit_hash',None); tb,vb=encode(tok,tr,a.max_length),encode(tok,va,a.max_length); base_loss=loss(base,vb)
 cfg=LoraConfig(r=8,lora_alpha=16,lora_dropout=0.0,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=['q_proj','v_proj'])
 model=get_peft_model(base,cfg); pars=[p for p in model.parameters() if p.requires_grad]; before=torch.cat([p.detach().flatten().clone() for p in pars]); opt=AdamW(pars,lr=a.lr); steps=[]
 model.train()
