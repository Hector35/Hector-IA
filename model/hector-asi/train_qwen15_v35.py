...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--output-dir',required=True);p.add_argument('--train-files',nargs='+',required=True);p.add_argument('--validation-files',nargs='+',required=True);p.add_argument('--experiment-id',default='hector-asi-qwen15-v35');p.add_argument('--epochs',type=int,default=2);p.add_argument('--max-length',type=int,default=112);p.add_argument('--lr',type=float,default=3e-5);p.add_argument('--warmup-ratio',type=float,default=.10);p.add_argument('--seed',type=int,default=35);return p.parse_args()
def sha(path):
 h=hashlib.sha256()
...
def read(paths):
 ...
 ids=[r['id'] for r in rows];prompts=[norm(m.get('content','')) for r in rows for m in r.get('messages',[]) if m.get('role')=='user']
...
def main():
 ...
 (tr,tp),(va,vp)=read(a.train_files),read(a.validation_files)
 if {r['id'] for r in tr}&{r['id'] for r in va} or set(tp)&set(vp):raise ValueError('split contamination')
 ...
 model=get_peft_model(base,cfg);pars=[p for p in model.parameters() if p.requires_grad];before=[p.detach().clone() for p in pars];opt=AdamW(pars,lr=a.lr,weight_decay=.01)
 total=a.epochs*len(tb);warmup=max(1,round(total*a.warmup_ratio));losses=[];lrs=[];model.train();step=0
 for _ in range(a.epochs):
  for b in tb:
   step+=1;current=a.lr*min(1.0,step/warmup)
   for g in opt.param_groups:g['lr']=current
   opt.zero_grad(set_to_none=True);z=model(**b).loss
   if not torch.isfinite(z):raise RuntimeError('non-finite loss')
   z.backward();torch.nn.utils.clip_grad_norm_(pars,1.0);opt.step();losses.append(float(z.detach().cpu()));lrs.append(current)
 delta=math.sqrt(sum(float(torch.sum((p.detach()-q)**2)) for p,q in zip(pars,before)));cand=avg_loss(model,vb)
 ...
 ds={'trainPaths':a.train_files,'trainSha256':{p:sha(p) for p in a.train_files},'trainExamples':len(tr),'validationPaths':a.validation_files,'validationSha256':{p:sha(p) for p in a.validation_files},'validationExamples':len(va),'crossSplitPromptOverlap':0,'privateUserData':False,'provenance':'author-generated Apache-2.0-compatible'}
 metrics={'schemaVersion':'1.0.0','experimentId':a.experiment_id,'status':'experimental-weights-generated','eligibleForChampion':False,'baseModel':{'repository':MODEL,'revision':REVISION,'license':'Apache-2.0'},'method':'LoRA SFT all-linear','dataset':ds,'hyperparameters':{'epochs':a.epochs,'maxLength':a.max_length,'learningRate':a.lr,'warmupRatio':a.warmup_ratio,'warmupSteps':warmup,'scheduler':'linear-warmup-then-constant','weightDecay':.01,'seed':a.seed,'loraRank':16,'loraAlpha':32,'loraDropout':.05,'targetModules':TARGETS},'results':{'steps':len(losses),'firstLearningRate':lrs[0],'peakLearningRate':max(lrs),'finalLearningRate':lrs[-1],'firstTrainLoss':losses[0],'finalTrainLoss':losses[-1],'baseValidationLoss':base_loss,'candidateValidationLoss':cand,'relativeValidationImprovement':(base_loss-cand)/base_loss,'parameterDeltaL2':delta,'trainableParameters':sum(p.numel() for p in pars),'weightsGenerated':True,'adapterBytes':weights.stat().st_size,'adapterSha256':sha(weights)},'runtime':{'seconds':time.time()-started,'python':platform.python_version(),'packages':{'torch':torch.__version__,'transformers':version('transformers'),'peft':version('peft'),'accelerate':version('accelerate'),'safetensors':version('safetensors')},'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}}
