...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--output-dir',required=True);p.add_argument('--train-files',nargs='+',required=True);p.add_argument('--validation-files',nargs='+',required=True);p.add_argument('--experiment-id',default='hector-asi-qwen15-v36');p.add_argument('--epochs',type=int,default=2);p.add_argument('--max-length',type=int,default=112);p.add_argument('--lr',type=float,default=3e-5);p.add_argument('--seed',type=int,default=35);p.add_argument('--gradient-accumulation',type=int,default=2);return p.parse_args()
def sha(path):
 h=hashlib.sha256()
...
def main():
 ...
 if a.gradient_accumulation<1:raise ValueError('gradient accumulation must be positive')
 ...
 if len(tr)%a.gradient_accumulation:raise ValueError('train size must divide gradient accumulation')
 ...
 model=get_peft_model(base,cfg);pars=[p for p in model.parameters() if p.requires_grad];before=[p.detach().clone() for p in pars];opt=AdamW(pars,lr=a.lr,weight_decay=.01,betas=(.9,.999),eps=1e-8,amsgrad=False,foreach=False);losses=[];optimizer_steps=0;model.train();opt.zero_grad(set_to_none=True)
 for _ in range(a.epochs):
  for index,b in enumerate(tb,1):
   z=model(**b).loss
   if not torch.isfinite(z):raise RuntimeError('non-finite loss')
   (z/a.gradient_accumulation).backward();losses.append(float(z.detach().cpu()))
   if index%a.gradient_accumulation==0:
    torch.nn.utils.clip_grad_norm_(pars,1.0);opt.step();opt.zero_grad(set_to_none=True);optimizer_steps+=1
 ...
 metrics={'schemaVersion':'1.0.0','experimentId':a.experiment_id,'status':'experimental-weights-generated','eligibleForChampion':False,'baseModel':{'repository':MODEL,'revision':REVISION,'license':'Apache-2.0'},'method':'LoRA SFT all-linear AdamW gradient-accumulation-2','dataset':ds,'hyperparameters':{'epochs':a.epochs,'maxLength':a.max_length,'learningRate':a.lr,'weightDecay':.01,'adamBetas':[.9,.999],'adamEpsilon':1e-8,'amsgrad':False,'gradientClip':1.0,'gradientAccumulation':a.gradient_accumulation,'effectiveBatchSize':a.gradient_accumulation,'seed':a.seed,'loraRank':16,'loraAlpha':32,'loraDropout':.05,'targetModules':TARGETS},'results':{'microSteps':len(losses),'optimizerSteps':optimizer_steps,'firstTrainLoss':losses[0],'finalTrainLoss':losses[-1],'baseValidationLoss':base_loss,'candidateValidationLoss':cand,'relativeValidationImprovement':(base_loss-cand)/base_loss,'parameterDeltaL2':delta,'trainableParameters':sum(p.numel() for p in pars),'weightsGenerated':True,'adapterBytes':weights.stat().st_size,'adapterSha256':sha(weights)},'runtime':{'seconds':time.time()-started,'python':platform.python_version(),'torch':torch.__version__,'hardware':'GitHub Actions ubuntu-latest CPU','githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}}
