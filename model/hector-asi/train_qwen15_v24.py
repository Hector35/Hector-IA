...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--output-dir',required=True);p.add_argument('--train-files',nargs='+',required=True);p.add_argument('--validation-files',nargs='+',required=True);p.add_argument('--experiment-id',default='hector-asi-qwen15-v24');p.add_argument('--epochs',type=int,default=2);p.add_argument('--max-length',type=int,default=112);p.add_argument('--lr',type=float,default=4e-5);p.add_argument('--seed',type=int,default=35);return p.parse_args()
def sha(path):
 h=hashlib.sha256()
...
def main():
 ...
 train_ids={r['id'] for r in tr};val_ids={r['id'] for r in va};train_prompts={' '.join(m.get('content','').lower().split()) for r in tr for m in r['messages'] if m.get('role')=='user'};val_prompts={' '.join(m.get('content','').lower().split()) for r in va for m in r['messages'] if m.get('role')=='user'}
