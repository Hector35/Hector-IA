...
def parse_args():
 p=argparse.ArgumentParser(); p.add_argument('--output-dir',default='artifacts/hector-asi-qwen15-v1'); p.add_argument('--train-file',default=DEFAULT_TRAIN); p.add_argument('--validation-file',default=DEFAULT_VALIDATION); p.add_argument('--experiment-id',default='hector-asi-qwen15-v1'); p.add_argument('--epochs',type=int,default=1); p.add_argument('--max-length',type=int,default=64); p.add_argument('--lr',type=float,default=2e-4); p.add_argument('--seed',type=int,default=35); return p.parse_args()
def read(path):
...
def main():
 ...
 tp={' '.join(m.get('content','').lower().split()) for r in tr for m in r['messages'] if m['role']=='user'}; vp={' '.join(m.get('content','').lower().split()) for r in va for m in r['messages'] if m['role']=='user'}
 if tp&vp: raise ValueError('normalized prompt contamination')
 ...
 cfg=LoraConfig(r=4,lora_alpha=8,lora_dropout=0.0,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=['q_proj','v_proj']); model=get_peft_model(base,cfg); pars=[p for p in model.parameters() if p.requires_grad]
 ...
 cand_loss=mean_loss(model,vb); adapter.mkdir(parents=True,exist_ok=True); model.save_pretrained(adapter,safe_serialization=True); tok.save_pretrained(adapter); weights=adapter/'adapter_model.safetensors'
 ...
 (root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True)+'\n',encoding='utf-8'); files=sorted(p for p in root.rglob('*') if p.is_file()); (root/'SHA256SUMS').write_text(''.join(f'{sha(p)}  {p.relative_to(root)}\n' for p in files),encoding='utf-8'); (root/'run-manifest.json').write_text(json.dumps({'experimentId':metrics['experimentId'],'files':{str(p.relative_to(root)):sha(p) for p in files},'rollback':'Do not load or delete adapter; frozen base remains unchanged.'},indent=2,sort_keys=True)+'\n',encoding='utf-8'); print(json.dumps(metrics,indent=2,sort_keys=True))
