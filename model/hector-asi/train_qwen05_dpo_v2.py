...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--train-file');p.add_argument('--eval-file');p.add_argument('--output-dir');p.add_argument('--epochs',type=int,default=4);p.add_argument('--max-length',type=int,default=160);p.add_argument('--lr',type=float,default=2e-4);p.add_argument('--beta',type=float,default=.2);p.add_argument('--seed',type=int,default=35);return p.parse_args()
def rows(path):
...
def pref(model,pairs):
 ...
 return {'accuracy':sum(x>0 for x in m)/len(m),'meanMargin':sum(m)/len(m),'minimumMargin':min(m)}
...
def main():
 ...
 model=get_peft_model(base,LoraConfig(r=4,lora_alpha=8,lora_dropout=0.0,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=['q_proj','v_proj']));pars=[p for p in model.parameters() if p.requires_grad];before=[p.detach().clone() for p in pars];opt=AdamW(pars,lr=a.lr);losses=[]
 for _ in range(a.epochs):
  model.train()
  for i,(c,r) in enumerate(train):
   opt.zero_grad(set_to_none=True);pc,pr=avg_logp(model,c),avg_logp(model,r);rc,rr=refs[i];loss=-F.logsigmoid(a.beta*((pc-pr)-(rc-rr))).mean();loss.backward();torch.nn.utils.clip_grad_norm_(pars,1.0);opt.step();losses.append(float(loss.detach()))
 ...
 metrics={'schemaVersion':'1.0.0','experimentId':'hector-asi-qwen05-dpo-v2','status':'experimental-preference-weights-generated','eligibleForChampion':False,'baseModel':{'repository':MODEL,'revision':REVISION,'license':'Apache-2.0'},'method':'length-normalized LoRA DPO','dataset':{'trainPath':a.train_file,'trainSha256':sha(a.train_file),'trainPairs':len(tr),'evalPath':a.eval_file,'evalSha256':sha(a.eval_file),'evalPairs':len(ev),'privateUserData':False,'splitContaminationChecked':True},'hyperparameters':{'epochs':a.epochs,'maxLength':a.max_length,'learningRate':a.lr,'beta':a.beta,'seed':a.seed,'rank':4,'alpha':8},'results':{'steps':len(losses),'firstDpoLoss':losses[0],'finalDpoLoss':losses[-1],'basePreferenceAccuracy':base_pref['accuracy'],'candidatePreferenceAccuracy':cand_pref['accuracy'],'baseMeanPreferenceMargin':base_pref['meanMargin'],'candidateMeanPreferenceMargin':cand_pref['meanMargin'],'baseLanguageLoss':base_loss,'candidateLanguageLoss':cand_loss,'languageLossRelativeChange':(cand_loss-base_loss)/base_loss,'parameterDeltaL2':delta,'trainableParameters':sum(p.numel() for p in pars),'weightsGenerated':True,'adapterBytes':w.stat().st_size,'adapterSha256':sha(w)},'promotionDecision':{'decision':'evaluate-only','blockedBy':'0.5B base and six hidden preference pairs are insufficient for champion promotion.'},'runtime':{'seconds':time.time()-start,'python':platform.python_version(),'torch':torch.__version__,'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}}
