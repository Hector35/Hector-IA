...
def main() -> None:
    ...
    quantization_config = BitsAndBytesConfig(
        load_in_4bit=bool(quant["loadIn4Bit"]),
        bnb_4bit_quant_type=quant["quantType"],
        bnb_4bit_use_double_quant=bool(quant["doubleQuant"]),
    ...
    training_args = SFTConfig(
        ...
        learning_rate=float(train_cfg["learningRate"]),
        per_device_train_batch_size=int(train_cfg["microBatchSize"]),
        per_device_eval_batch_size=1,
        gradient_accumulation_steps=int(train_cfg["gradientAccumulationSteps"]),
        warmup_ratio=float(train_cfg["warmupRatio"]),
        weight_decay=float(train_cfg["weightDecay"]),
        max_length=int(train_cfg["maxSequenceLength"]),
        packing=bool(train_cfg["packing"]),
        gradient_checkpointing=bool(train_cfg["gradientCheckpointing"]),
        bf16=dtype == torch.bfloat16,
        fp16=dtype == torch.float16,
        eval_strategy="steps",
        save_strategy="steps",
        save_steps=int(train_cfg["saveSteps"]),
        eval_steps=int(train_cfg["evalSteps"]),
        logging_steps=int(train_cfg["loggingSteps"]),
        save_total_limit=3,
    ...
    manifest = {
        ...
        "inputs": {
            ...
            "trainSha256": file_sha256(Path(args.train)),
            "validationSha256": file_sha256(Path(args.validation)),
            "hiddenSha256": file_sha256(Path(args.hidden)),
        },
        "adapter": {
            "path": str(adapter),
