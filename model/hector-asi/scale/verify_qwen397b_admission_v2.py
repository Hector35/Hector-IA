...
def main() -> None:
    ...
    gates = {
        ...
        'costWithinBudget': isinstance(obs['budgetMaximumMxn'], (int, float)) and isinstance(obs['estimatedMaximumCostMxn'], (int, float)) and 0 <= obs['estimatedMaximumCostMxn'] <= obs['budgetMaximumMxn'],
        'liveExactModelAttestation': bool(obs['liveExactModelAttested']),
    }
    eligible = all(gates.values())
    evidence = {'schemaVersion':4,'baseModel':manifest['baseModel'],'champion':manifest['champion'],'eligible':eligible,'gates':gates,'missing':[name for name,opened in gates.items() if not opened],'decision':'train' if eligible else 'do-not-train','artifactAllowed':eligible,'observed':obs}
    ...
    output=Path(args.output);output.parent.mkdir(parents=True,exist_ok=True);output.write_text(json.dumps(evidence,indent=2,sort_keys=True)+'\n',encoding='utf-8')
