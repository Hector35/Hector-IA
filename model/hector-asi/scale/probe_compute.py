...
def gpu_info() -> list[dict[str, object]]:
    ...
    for index, line in enumerate(raw.splitlines()):
        ...
        result.append({"index": index, "name": name, "memoryTotalMiB": int(total), "memoryFreeMiB": int(free), "driver": driver})
    return result
...
def main() -> None:
    ...
    payload = {
        ...
        "host": {"platform": platform.platform(), "python": platform.python_version(), "cpuCount": os.cpu_count(), "ramTotalBytes": None, "diskFreeBytes": disk.free},
        "cuda": {"nvidiaSmi": command(["nvidia-smi", "--query-gpu=driver_version", "--format=csv,noheader"]), "nvcc": command(["nvcc", "--version"]), "gpus": gpus},
        "admission": {
