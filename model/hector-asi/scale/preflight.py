...
def main() -> None:
    ...
    parser.add_argument("--resume-probe", required=True)
    parser.add_argument("--output", required=True)
    args = parser.parse_args()
    paths = {name: Path(getattr(args, name.replace('-', '_'))) for name in ("config", "train", "validation", "hidden", "probe", "resume-probe")}
    missing = [str(path) for path in paths.values() if not path.is_file()]
    if missing:
        raise SystemExit(f"Missing required inputs: {missing}")
    config = json.loads(paths["config"].read_text())
    probe = json.loads(paths["probe"].read_text())
    resume_probe = json.loads(paths["resume-probe"].read_text())
    ...
    gates = {
        ...
        "resumeRoundTrip": bool(resume_probe.get("ok", False)),
    ...
    payload = {
        ...
        "command": ["python", "model/hector-asi/scale/train_qlora_main.py", "--config", str(paths["config"]), "--train", str(paths["train"]), "--validation", str(paths["validation"]), "--hidden", str(paths["hidden"]), "--output", f"$HECTOR_TRAINING_ROOT/{profile}", "--resume"],
    }
