...
def norm(text:str)->str:return re.sub(r'\d+','#',re.sub(r'\s+',' ',text.lower())).strip()
...
def main():
 ...
 errors=[];ids=[];prompts=[];semantic=[];cats=Counter();difficulties=Counter()
 for i,row in enumerate(rows,1):
  ...
  ids.append(row.get('id'));prompts.append(row.get('prompt',''));semantic.append(norm(row.get('prompt','')));cats[row.get('category')]+=1;difficulties[row.get('difficulty')]+=1
  ...
  # Generator uses default JSON spacing; verify either canonical or generator serialization.
  copy2=dict(row);copy2['content_sha256']=None
  generator_actual=sha256(json.dumps(copy2,ensure_ascii=False).encode())
  if expected not in (actual,generator_actual):errors.append(f'row {i}: content hash mismatch')
 ...
 # Semantic templates are expected, but every category must have >=10 distinct normalized structures.
 for cat in CATEGORIES:
  values={norm(r['prompt']) for r in rows if r['category']==cat}
  if len(values)<10:errors.append(f'{cat}: insufficient semantic diversity {len(values)}')
