...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--output-dir',required=True);p.add_argument('--train-files',nargs='+',required=True);p.add_argument('--validation-files',nargs='+',required=True);p.add_argument('--test-files',nargs='+',required=True);p.add_argument('--experiment-id',default='hector-asi-qwen15-v40');p.add_argument('--epochs',type=int,default=3);p.add_argument('--max-length',type=int,default=160);p.add_argument('--lr',type=float,default=3e-5);p.add_argument('--seed',type=int,default=35);return p.parse_args()
def sha(path):
 h=hashlib.sha256()
...
def read(paths):
 ...
 return rows
def signatures(rows):
 return {r['id'] for r in rows},{' '.join(m.get('content','').lower().split()) for r in rows for m in r.get('messages',[]) if m.get('role')=='user'}
...
def main():
 ...
 cfg=LoraConfig(r=16,lora_alpha=32,lora_dropout=.05,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=TARGETS);model=get_peft_model(base,cfg);pars=[p for p in model.parameters() if p.requires_grad];before={n:p.detach().cpu().clone() for n,p in model.named_parameters() if p.requires_grad};opt=AdamW(pars,lr=a.lr,betas=(.9,.999),weight_decay=.01,foreach=False)
 history=[];best=None;best_epoch=0;best_val=float('inf')
 for epoch in range(1,a.epochs+1):
  ...
  val=avg_loss(model,vb);history.append({'epoch':epoch,'meanTrainLoss':sum(losses)/len(losses),'validationLoss':val})
  if val<best_val:
   best_val=val;best_epoch=epoch;best={k:v.detach().cpu().clone() for k,v in model.state_dict().items() if 'lora_' in k}
 if best is None:raise RuntimeError('no checkpoint selected')
 set_peft_model_state_dict(model,best);cand_test=avg_loss(model,eb);delta=math.sqrt(sum(float(torch.sum((p.detach().cpu()-before[n])**2)) for n,p in model.named_parameters() if p.requires_grad and n in before))
