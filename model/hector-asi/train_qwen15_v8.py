...
def args():
 p=argparse.ArgumentParser(); p.add_argument('--output-dir',required=True); p.add_argument('--train-file',required=True); p.add_argument('--validation-file',required=True); p.add_argument('--experiment-id',default='hector-asi-qwen15-v8'); p.add_argument('--epochs',type=int,default=2); p.add_argument('--max-length',type=int,default=112); p.add_argument('--lr',type=float,default=5e-5); p.add_argument('--seed',type=int,default=35); return p.parse_args()
...
def encode(tok,rows,n):
 ...
 for row in rows:
  ...
  b=tok(text,return_tensors='pt',truncation=True,max_length=n); b['labels']=b['input_ids'].clone(); out.append(b)
 return out
...
def main():
 ...
 cfg=LoraConfig(r=32,lora_alpha=64,lora_dropout=0.0,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=['q_proj','k_proj','v_proj','o_proj'])
 ...
 delta=float(torch.linalg.vector_norm(torch.cat([p.detach().flatten() for p in pars])-before)); cand=mean_loss(model,vb)
