...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--output-dir',required=True);p.add_argument('--train-files',nargs='+',required=True);p.add_argument('--validation-files',nargs='+',required=True);p.add_argument('--experiment-id',default='hector-asi-qwen15-v26');p.add_argument('--epochs',type=int,default=2);p.add_argument('--max-length',type=int,default=112);p.add_argument('--lr',type=float,default=3e-5);p.add_argument('--seed',type=int,default=35);return p.parse_args()
def sha(path):
 h=hashlib.sha256()
 with open(path,'rb') as f:
  for c in iter(lambda:f.read(1<<20),b''):h.update(c)
 return h.hexdigest()
def norm_prompt(row):return ' '.join(next(m['content'] for m in row['messages'] if m.get('role')=='user').lower().split())
def read(paths):
 rows=[]
 ...
 ids=[r['id'] for r in rows];prompts=[norm_prompt(r) for r in rows]
...
def main():
 ...
 tr,va=read(a.train_files),read(a.validation_files);train_ids={r['id'] for r in tr};val_ids={r['id'] for r in va};train_prompts={norm_prompt(r) for r in tr};val_prompts={norm_prompt(r) for r in va}
 ...
 model=get_peft_model(base,cfg);pars=[p for p in model.parameters() if p.requires_grad];before=torch.cat([p.detach().flatten().clone() for p in pars]);opt=AdamW(pars,lr=a.lr,weight_decay=.02);losses=[];model.train()
