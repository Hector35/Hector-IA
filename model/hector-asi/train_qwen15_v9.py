...
def args():
 p=argparse.ArgumentParser(); p.add_argument('--output-dir',required=True); p.add_argument('--train-file',required=True); p.add_argument('--validation-file',required=True); p.add_argument('--experiment-id',default='hector-asi-qwen15-v9'); p.add_argument('--epochs',type=int,default=2); p.add_argument('--max-length',type=int,default=112); p.add_argument('--lr',type=float,default=3e-5); p.add_argument('--seed',type=int,default=35); return p.parse_args()
def read(path):
...
def main():
 ...
 cfg=LoraConfig(r=64,lora_alpha=128,lora_dropout=0.0,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=['q_proj','k_proj','v_proj','o_proj'])
