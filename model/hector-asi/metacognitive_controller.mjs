...
export function naiveController(input) {
  const confidence = input.attempts > 0 ? clamp(input.successes / input.attempts) : 0.8;
  const action = input.highRisk ? 'escalate' : 'answer';
  return { confidence, action, changedStrategy: false, stopped: false, detectedFailure: false, rationale: 'Baseline barato: responde salvo riesgo alto.' };
}
...
export function selectiveController(input) {
  ...
  let stopped = false;
  const detectedFailure = Boolean(input.negativeEvidence || (input.repeatedFailures ?? 0) > 0);
  ...
  if (input.goalSatisfied && input.validationPassed) {
    ...
    action = 'revise'; changedStrategy = true;
  } else if (input.highRisk && !input.canVerify) {
    action = input.masterAvailable ? 'escalate' : 'abstain'; stopped = true;
  } else if (input.currentInformationRequired && !input.canVerify) {
    action = 'abstain'; stopped = true;
  } else if (input.highRisk || confidence < 0.72 || input.attempts < 5) {
    action = 'verify';
  }

  return { confidence, action, changedStrategy, stopped, detectedFailure, rationale: `Control selectivo: evidencia=${input.successes ?? 0}/${input.attempts ?? 0}, riesgo=${Boolean(input.highRisk)}, fallos=${input.repeatedFailures ?? 0}.` };
}
...
function scoreRow(test, output) {
  ...
  return {
    ...
    actionCorrect: action === test.expectedAction,
...
export function evaluateController(cases, controller) {
  ...
  return {
    schemaVersion: '1.0.0', caseCount: cases.length,
    ...
    estimatedApiCostUsd: 0, latencyMs, rows,
  };
}
...
export function evaluateIncrementally(cases, outputsById, previousCache = {}) {
  ...
  return { cache, rows, recomputed, reused: cases.length - recomputed, reuseRate: cases.length ? (cases.length - recomputed) / cases.length : 1 };
}
...
export function compareApproaches(cases) {
  ...
  const candidateWins = candidate.actionSelectionAccuracy > baseline.actionSelectionAccuracy && candidate.brierScore < baseline.brierScore && candidate.unnecessaryEscalationRate <= baseline.unnecessaryEscalationRate;
  return { baseline, candidate, decision: candidateWins ? 'keep-selective-controller' : 'kill-selective-controller' };
}
