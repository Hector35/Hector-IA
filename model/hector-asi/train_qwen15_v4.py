...
def args():
 p=argparse.ArgumentParser(); p.add_argument('--output-dir',required=True); p.add_argument('--train-file',required=True); p.add_argument('--validation-file',required=True); p.add_argument('--experiment-id',default='hector-asi-qwen15-v4'); p.add_argument('--epochs',type=int,default=2); p.add_argument('--max-length',type=int,default=96); p.add_argument('--lr',type=float,default=1e-4); p.add_argument('--seed',type=int,default=35); return p.parse_args()
def read(path):
...
def main():
 ...
 cfg=LoraConfig(r=8,lora_alpha=16,lora_dropout=0.0,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=['q_proj','k_proj','v_proj','o_proj'])
 model=get_peft_model(base,cfg); pars=[p for p in model.parameters() if p.requires_grad]; before=torch.cat([p.detach().flatten().clone() for p in pars]); opt=AdamW(pars,lr=a.lr); losses=[]; model.train()
