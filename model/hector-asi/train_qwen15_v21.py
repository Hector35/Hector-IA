...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--output-dir',required=True);p.add_argument('--train-files',nargs='+',required=True);p.add_argument('--validation-files',nargs='+',required=True);p.add_argument('--experiment-id',default='hector-asi-qwen15-v21');p.add_argument('--epochs',type=int,default=2);p.add_argument('--max-length',type=int,default=112);p.add_argument('--lr',type=float,default=3e-5);p.add_argument('--seed',type=int,default=35);return p.parse_args()
def sha(path):
 h=hashlib.sha256()
 with open(path,'rb') as f:
  for c in iter(lambda:f.read(1<<20),b''):h.update(c)
 return h.hexdigest()
def prompt_key(row):return ' '.join(next(m['content'] for m in row['messages'] if m['role']=='user').lower().split())
def read(paths):
 rows=[]
...
def main():
 ...
 if {r['id'] for r in tr}&{r['id'] for r in va} or {prompt_key(r) for r in tr}&{prompt_key(r) for r in va}:raise ValueError('split contamination')
 ...
 cfg=LoraConfig(r=16,lora_alpha=32,lora_dropout=.10,bias='none',task_type=TaskType.CAUSAL_LM,target_modules=TARGETS)
