...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--epochs',type=int,default=2);p.add_argument('--lr',type=float,default=1.5e-6);p.add_argument('--seed',type=int,default=3062);p.add_argument('--max-length',type=int,default=144);p.add_argument('--replay-scale',type=float,default=.65);p.add_argument('--eos-lambda',type=float,default=.15);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def main():
 ...
 tr,va,hi=rows(a.train),rows(a.validation),hidden(a.hidden);ss=[{norm(x['prompt']) for x in z} for z in (tr,va,hi)];assert not ss[0]&ss[1] and not ss[0]&ss[2] and not ss[1]&ss[2];f=[x for x in tr if x['group']=='focal'];r=[x for x in tr if x['group']=='replay'];assert len(f)==len(r)==8
 ...
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for bf,br in zip(fb,rb):
   opt.zero_grad(set_to_none=True);lf=loss(m,bf,a.eos_lambda);gf=torch.autograd.grad(lf,pars,retain_graph=False,allow_unused=False);lr=loss(m,br,a.eos_lambda);gr=torch.autograd.grad(lr,pars,retain_graph=False,allow_unused=False);dot=sum(torch.sum(x*y) for x,y in zip(gf,gr));nr=sum(torch.sum(y*y) for y in gr)
   if dot<0:gf=[x-dot/(nr+1e-12)*y for x,y in zip(gf,gr)];conflicts+=1
   for p,x,y in zip(pars,gf,gr):p.grad=x+a.replay_scale*y
   torch.nn.utils.clip_grad_norm_(pars,.5);opt.step();ls.append(float(lf.detach()+a.replay_scale*lr.detach()))
