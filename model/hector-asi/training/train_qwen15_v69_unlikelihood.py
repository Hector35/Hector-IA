...
def unlikelihood(model,b):
 o=model(input_ids=b['input_ids'],attention_mask=b['attention_mask']);lg=o.logits[:,:-1];la=b['labels'][:,1:];mask=la!=-100;probs=torch.softmax(lg[mask],dim=-1);p=probs.gather(1,la[mask].unsqueeze(1)).squeeze(1).clamp(max=.999999);return b['weight']*(-torch.log1p(-p)).mean()
def objective(model,pair,eos_l,ul_l):return chosen_ce(model,pair[0],eos_l)+ul_l*unlikelihood(model,pair[1])
def meanloss(model,pairs,eos_l,ul_l):
 model.eval();v=[]
 with torch.no_grad():
  for p in pairs:v.append(float(objective(model,p,eos_l,ul_l)))
 return sum(v)/len(v)
...
def main():
 ...
 tok=AutoTokenizer.from_pretrained(src,use_fast=True);tok.pad_token=tok.pad_token or tok.eos_token;tb=[(encode(tok,x,a.max_length,'chosen'),encode(tok,x,a.max_length,'rejected')) for x in tr];vb=[(encode(tok,x,a.max_length,'chosen'),encode(tok,x,a.max_length,'rejected')) for x in va]
 b=base();champ=PeftModel.from_pretrained(b,src,is_trainable=False);before=meanloss(champ,vb,a.eos_lambda,a.ul_lambda);bb=behavior(champ,tok,hi);del champ,b;gc.collect()
 b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);pars=[p for n,p in m.named_parameters() if p.requires_grad and 'lora_' in n];opt=AdamW(pars,lr=a.lr,weight_decay=.015,foreach=False);root=Path(a.out);final=root/'adapter';root.mkdir(parents=True,exist_ok=True);hist=[]
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for pair in tb:
   opt.zero_grad(set_to_none=True);loss=objective(m,pair,a.eos_lambda,a.ul_lambda);loss.backward();torch.nn.utils.clip_grad_norm_(pars,.15);opt.step();ls.append(float(loss.detach()))
  hist.append({'epoch':ep,'trainObjective':sum(ls)/len(ls),'peakRssMb':rss()})
 m.save_pretrained(final,safe_serialization=True);tok.save_pretrained(final);after=meanloss(m,vb,a.eos_lambda,a.ul_lambda);ab=behavior(m,tok,hi);wf=final/'adapter_model.safetensors';lat=ab['meanLatencyMs']/max(1,bb['meanLatencyMs']);tl=ab['meanTokens']/max(1,bb['meanTokens']);g={'validationImproved':after<before,'behavioralImprovement':ab['score']>bb['score'],'criticalPerfect':ab['criticalPassed']==ab['criticalTotal'],'noCriticalRegression':ab['criticalPassed']>=bb['criticalPassed'],'latencyAcceptable':lat<=1.15,'tokenLengthAcceptable':tl<=1.05};g['eligibleForPromotion']=all(g.values());metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v69-unlikelihood','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'chosen response CE plus sequence-level rejected-token unlikelihood with replay','dataset':{'trainRows':len(tr),'focalRows':8,'replayRows':16,'validationRows':len(va),'hiddenRows':len(hi),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lr':a.lr,'unlikelihoodLambda':a.ul_lambda,'eosLambda':a.eos_lambda,'seed':a.seed,'maxLength':a.max_length},'baseline':{'validationObjective':before,'behavioral':bb},'candidate':{'validationObjective':after,'behavioral':ab,'latencyRatio':lat,'tokenRatio':tl,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-start,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}};(root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False)+'\n');print(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False))
