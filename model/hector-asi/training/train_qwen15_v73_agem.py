...
def setgrad(params,v):
 off=0
 for p in params:
  n=p.numel();p.grad=v[off:off+n].view_as(p).clone();off+=n
def main():
 ...
 tr,va,hi=rows(a.train),rows(a.validation),hidden(a.hidden);ss=[{norm(x['prompt']) for x in z} for z in (tr,va,hi)];assert not ss[0]&ss[1] and not ss[0]&ss[2] and not ss[1]&ss[2];f=[x for x in tr if x['group']=='focal'];r=[x for x in tr if x['group']=='replay'];assert len(f)==12 and len(r)==20
 ...
 for n,p in m.named_parameters():
  layer=re.search(r'layers\.(\d+)\.',n);idx=int(layer.group(1)) if layer else -1;keep=18<=idx<=27 and ('lora_A' in n or 'lora_B' in n);p.requires_grad=keep
  if keep:params.append(p)
 assert params;opt=AdamW(params,lr=a.lr,weight_decay=.035,foreach=False);root=Path(a.out);final=root/'adapter';root.mkdir(parents=True,exist_ok=True);hist=[]
 for ep in range(1,a.epochs+1):
  m.train();ls=[];conflicts=0
  for i,bf in enumerate(fb):
   br=rb[(i+(ep-1)*len(fb))%len(rb)];opt.zero_grad(set_to_none=True);lr=objective(m,br,a.eos_lambda);lr.backward();gr=gradvec(params).detach();opt.zero_grad(set_to_none=True);lf=objective(m,bf,a.eos_lambda);lf.backward();gf=gradvec(params);dot=torch.dot(gf,gr)
   if dot<0:
    gf=gf-dot*gr/(torch.dot(gr,gr)+1e-12);setgrad(params,gf);conflicts+=1
   torch.nn.utils.clip_grad_norm_(params,.08);opt.step();ls.append(float(lf.detach()))
  hist.append({'epoch':ep,'focalObjective':sum(ls)/len(ls),'projectedConflicts':conflicts,'peakRssMb':rss()})
 m.save_pretrained(final,safe_serialization=True);tok.save_pretrained(final);after=meanloss(m,vb,a.eos_lambda);ab=behavior(m,tok,hi);wf=final/'adapter_model.safetensors';lat=ab['meanLatencyMs']/max(1,bb['meanLatencyMs']);tl=ab['meanTokens']/max(1,bb['meanTokens']);g={'validationImproved':after<before,'behavioralImprovement':ab['score']>bb['score'],'criticalPerfect':ab['criticalPassed']==ab['criticalTotal'],'noCriticalRegression':ab['criticalPassed']>=bb['criticalPassed'],'latencyAcceptable':lat<=1.15,'tokenLengthAcceptable':tl<=1.05};g['eligibleForPromotion']=all(g.values());metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v73-agem','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'A-GEM constrained LoRA: focal gradients projected against replay gradients, lora_A/B layers 18-27','dataset':{'trainRows':len(tr),'focalRows':len(f),'replayRows':len(r),'validationRows':len(va),'hiddenRows':len(hi),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lr':a.lr,'eosLambda':a.eos_lambda,'seed':a.seed,'maxLength':a.max_length,'trainable':'lora_A and lora_B layers 18-27','gradientClip':.08,'weightDecay':.035},'baseline':{'validationObjective':before,'behavioral':bb},'candidate':{'validationObjective':after,'behavioral':ab,'latencyRatio':lat,'tokenRatio':tl,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-start,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}};(root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False)+'\n');print(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False))
