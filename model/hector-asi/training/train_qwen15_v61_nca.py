...
SYS='Responde en español con precisión, brevedad, evidencia, verificación y rechazo seguro de solicitudes dañinas.'
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--seed',type=int,default=3061);p.add_argument('--epochs',type=int,default=2);p.add_argument('--lr',type=float,default=1.5e-6);p.add_argument('--max-length',type=int,default=160);p.add_argument('--beta',type=float,default=.18);p.add_argument('--ce-weight',type=float,default=.45);p.add_argument('--pref-weight',type=float,default=.55);p.add_argument('--anchor',type=float,default=.03);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def seq(model,b):
 o=model(input_ids=b['input_ids'],attention_mask=b['attention_mask']);lg=o.logits[:,:-1];la=b['labels'][:,1:];mask=la!=-100;t=la.clamp_min(0);lp=F.log_softmax(lg,dim=-1).gather(-1,t.unsqueeze(-1)).squeeze(-1)[mask];return lp.mean()
def nca(model,bs,beta,cew,pw):
 vals=[]
 for r,c,u in bs:
  pc,pu=seq(model,c),seq(model,u);ce=-pc;contrast=-F.logsigmoid(beta*(pc-pu));vals.append(float(r.get('weight',1))*(cew*ce+pw*contrast))
...
def main():
 ...
 b=base();champ=PeftModel.from_pretrained(b,src,is_trainable=False);before=float(nca(champ,vb,a.beta,a.ce_weight,a.pref_weight));bb=behavior(champ,tok,hi);del champ,b;gc.collect()
 b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);pars=[p for p in m.parameters() if p.requires_grad];initial=[p.detach().clone() for p in pars];opt=AdamW(pars,lr=a.lr,weight_decay=.01,foreach=False);root=Path(a.out);bestdir=root/'best-adapter';final=root/'adapter';root.mkdir(parents=True,exist_ok=True);best=float('inf');hist=[]
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for item in tb:
   opt.zero_grad(set_to_none=True);loss=nca(m,[item],a.beta,a.ce_weight,a.pref_weight);anch=sum(torch.mean((p-q)**2) for p,q in zip(pars,initial))/len(pars);loss=loss+a.anchor*anch;assert torch.isfinite(loss);loss.backward();torch.nn.utils.clip_grad_norm_(pars,.5);opt.step();ls.append(float(loss.detach()))
  m.eval();v=float(nca(m,vb,a.beta,a.ce_weight,a.pref_weight));hist.append({'epoch':ep,'trainObjective':sum(ls)/len(ls),'validationObjective':v,'peakRssMb':rss()})
  if v<best:best=v;m.save_pretrained(bestdir,safe_serialization=True);tok.save_pretrained(bestdir)
 del m,b,opt,pars,initial;gc.collect();b=base();m=PeftModel.from_pretrained(b,bestdir,is_trainable=False);ab=behavior(m,tok,hi);m.save_pretrained(final,safe_serialization=True);tok.save_pretrained(final);wf=final/'adapter_model.safetensors';lat=ab['meanLatencyMs']/max(1,bb['meanLatencyMs']);tl=ab['meanTokens']/max(1,bb['meanTokens']);g={'validationImproved':best<before,'behavioralImprovement':ab['score']>bb['score'],'criticalPerfect':ab['criticalPassed']==ab['criticalTotal'],'noCriticalRegression':ab['criticalPassed']>=bb['criticalPassed'],'latencyAcceptable':lat<=1.15,'tokenLengthAcceptable':tl<=1.05};g['eligibleForPromotion']=all(g.values());metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v61-nca','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'NCA-style response-only chosen CE plus length-normalized chosen/rejected contrast and LoRA trust penalty','dataset':{'trainRows':len(tr),'validationRows':len(va),'hiddenRows':len(hi),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lr':a.lr,'beta':a.beta,'ceWeight':a.ce_weight,'preferenceWeight':a.pref_weight,'anchorLambda':a.anchor,'seed':a.seed,'maxLength':a.max_length},'baseline':{'validationObjective':before,'behavioral':bb},'candidate':{'validationObjective':best,'behavioral':ab,'latencyRatio':lat,'tokenRatio':tl,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-start,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}};(root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False)+'\n');print(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False))
