...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',nargs='+',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--seed',type=int,default=3055);p.add_argument('--epochs',type=int,default=2);p.add_argument('--lr',type=float,default=2e-6);p.add_argument('--max-length',type=int,default=160);p.add_argument('--beta',type=float,default=.20);p.add_argument('--anchor',type=float,default=.03);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def read_pairs(paths):
 ...
 assert all(r.get('verified') is True and r.get('provenance')=='author-generated' and r.get('license')=='Apache-2.0-compatible' for r in rows);return rows
def read_hidden(p):
 rows=[json.loads(x) for x in Path(p).read_text().splitlines() if x.strip()];assert len(rows)==18 and len({r['id'] for r in rows})==18;return rows
def base():
...
def mean_logp(model,b):
 o=model(input_ids=b['input_ids'],attention_mask=b['attention_mask']);logits=o.logits[:,:-1];labels=b['labels'][:,1:];mask=labels!=-100;targets=labels.clamp_min(0);lp=F.log_softmax(logits,dim=-1).gather(-1,targets.unsqueeze(-1)).squeeze(-1);return lp[mask].mean(),int(mask.sum())
def margins(model,bs):
 ...
 return out
def ipo_objective(model,bs,refs,beta):
 vals=[];target=1/(2*beta)
 with torch.no_grad():
  for (r,c,u),ref in zip(bs,refs):
   cl,_=mean_logp(model,c);rl,_=mean_logp(model,u);adv=(cl-rl)-ref['margin'];vals.append(float(((adv-target)**2*float(r.get('weight',1))).cpu()))
...
def main():
 ...
 b=base();champ=PeftModel.from_pretrained(b,src,is_trainable=False);tr_ref=margins(champ,tb);va_ref=margins(champ,vb);before_val=ipo_objective(champ,vb,va_ref,a.beta);before_beh=behavior(champ,tok,hi);del champ,b;gc.collect()
 b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);pars=[p for p in m.parameters() if p.requires_grad];initial=[p.detach().clone() for p in pars];opt=AdamW(pars,lr=a.lr,betas=(.9,.999),weight_decay=.01,foreach=False);root=Path(a.out);bestdir=root/'best-adapter';final=root/'adapter';root.mkdir(parents=True,exist_ok=True);best=float('inf');hist=[];target=1/(2*a.beta)
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for (r,c,u),ref in zip(tb,tr_ref):
   opt.zero_grad(set_to_none=True);cl,_=mean_logp(m,c);rl,_=mean_logp(m,u);adv=(cl-rl)-ref['margin'];pref=(adv-target)**2*float(r.get('weight',1));anch=sum(torch.mean((p-q)**2) for p,q in zip(pars,initial))/len(pars);loss=pref+a.anchor*anch
   if not torch.isfinite(loss):raise RuntimeError('non-finite loss')
   loss.backward();torch.nn.utils.clip_grad_norm_(pars,.5);opt.step();ls.append(float(loss.detach()))
  v=ipo_objective(m,vb,va_ref,a.beta);hist.append({'epoch':ep,'trainObjective':sum(ls)/len(ls),'validationObjective':v,'peakRssMb':rss()})
 ...
 metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v55-ipo','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'reference-anchored IPO squared preference objective with LoRA trust penalty','dataset':{'trainRows':len(tr),'validationRows':len(va),'hiddenRows':len(hi),'trainSha256':{p:sha(p) for p in a.train},'validationSha256':sha(a.validation),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lr':a.lr,'beta':a.beta,'ipoTarget':target,'anchorLambda':a.anchor,'clipNorm':.5,'seed':a.seed,'maxLength':a.max_length,'optimizer':'AdamW'},'baseline':{'validationObjective':before_val,'behavioral':before_beh},'candidate':{'validationObjective':best,'behavioral':after_beh,'latencyRatio':lat,'tokenRatio':tokratio,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-started,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}}
