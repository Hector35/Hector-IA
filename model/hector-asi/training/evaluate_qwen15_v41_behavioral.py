...
V15_SHA='ef9d36b288e100bbcddbab475758699416908627a59206c9a82f60ce711d66dc'; V41_SHA='31bead5f59982d8e321517fd235b81f992f83d2e3792a55690c2a841ff0e28f8'
def sha(p):
 d=hashlib.sha256()
 with p.open('rb') as f:
...
def main():
 ...
 repo=Path(os.getenv('GITHUB_WORKSPACE','.')); p=repo/'model/hector-asi/evals/qwen15-behavioral-v41.jsonl'; cs=load_cases(p)
 d15=Path(os.environ['V15_DIR']); d41=Path(os.environ['V41_DIR']); assert sha(d15/'adapter_model.safetensors')==V15_SHA; assert sha(d41/'adapter_model.safetensors')==V41_SHA
 ...
 del v15,base; base=AutoModelForCausalLM.from_pretrained(BASE,revision=REV,torch_dtype=torch.float32,low_cpu_mem_usage=True,attn_implementation='eager'); v41=PeftModel.from_pretrained(base,d41,is_trainable=False); v41.eval(); b=run('v41',v41,t,cs)
 ...
 report={'experimentId':'hector-asi-qwen15-v41','source':'hector-asi-qwen15-v15','adapterSha256':V41_SHA,'datasetSha256':sha(p),'hiddenTestLoss':1.028577556212743,'relativeHiddenImprovement':0.7413797562417694,'v15':a,'v41':b,'latencyRatio':ratio,'gates':gates,'decision':'promote' if gates['eligibleForPromotion'] else 'reject-or-hold','rollback':'hector-asi-qwen15-v15'}
 Path(os.getenv('EVAL_OUTPUT','/tmp/qwen15-v41-behavioral.json')).write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n'); print(json.dumps({'decision':report['decision'],'gates':gates}))
