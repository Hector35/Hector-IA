...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--epochs',type=int,default=3);p.add_argument('--lr',type=float,default=1.5e-6);p.add_argument('--seed',type=int,default=3065);p.add_argument('--max-length',type=int,default=144);p.add_argument('--distill-lambda',type=float,default=.70);p.add_argument('--replay-ce',type=float,default=.15);p.add_argument('--eos-lambda',type=float,default=.08);p.add_argument('--first-layer',type=int,default=16);p.add_argument('--last-layer',type=int,default=23);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def ce_loss(model,b,eos_l):
 o=model(input_ids=b['input_ids'],attention_mask=b['attention_mask']);lg=o.logits[:,:-1];la=b['labels'][:,1:];mask=la!=-100;ce=torch.nn.functional.cross_entropy(lg[mask],la[mask]);last=torch.where(mask[0])[0][-1];eos=torch.nn.functional.cross_entropy(lg[:,last],la[:,last]);return ce+eos_l*eos
...
def main():
 ...
 tr,va,hi=rows(a.train),rows(a.validation),hidden(a.hidden);ss=[{norm(x['prompt']) for x in z} for z in (tr,va,hi)];assert not ss[0]&ss[1] and not ss[0]&ss[2] and not ss[1]&ss[2];assert sum(x['group']=='focal' for x in tr)==8 and sum(x['group']=='replay' for x in tr)==12
 ...
 pars=[p for p in m.parameters() if p.requires_grad];assert pars and active;opt=AdamW(pars,lr=a.lr,weight_decay=.01,foreach=False);root=Path(a.out);bestdir=root/'best-adapter';final=root/'adapter';root.mkdir(parents=True,exist_ok=True);best=float('inf');hist=[]
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for i in range(max(len(focal),len(replay))):
   opt.zero_grad(set_to_none=True);fb=focal[i%len(focal)];rb=replay[i%len(replay)];lf=ce_loss(m,fb,a.eos_lambda);o=m(input_ids=rb['input_ids'],attention_mask=rb['attention_mask']).logits[:,:-1];ref=refs[i%len(replay)].to(o.dtype);mask=rb['labels'][:,1:]!=-100;kl=torch.nn.functional.kl_div(torch.log_softmax(o[mask],-1),torch.softmax(ref[mask],-1),reduction='batchmean');lr=ce_loss(m,rb,a.eos_lambda);loss=lf+a.distill_lambda*kl+a.replay_ce*lr;loss.backward();torch.nn.utils.clip_grad_norm_(pars,.30);opt.step();ls.append(float(loss.detach()))
