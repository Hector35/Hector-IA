...
def main():
 ...
 pars=[p for p in model.parameters() if p.requires_grad];snapshot=torch.cat([p.detach().flatten().cpu().clone() for p in pars]);opt=AdamW(pars,lr=a.lr,betas=(.9,.999),weight_decay=.01,foreach=False);model.train();losses=[]
 for b in tb:
 ...
 after=avg_loss(model,vb);current=torch.cat([p.detach().flatten().cpu() for p in pars]);delta=float(torch.linalg.vector_norm(current-snapshot))
 ...
 manifest={'schemaVersion':'1.0.0','experimentId':'hector-asi-qwen15-v49','status':'experimental-weights-generated','eligibleForChampion':False,'baseModel':{'repository':MODEL,'revision':REVISION,'license':'Apache-2.0'},'sourceChampion':{'id':'hector-asi-qwen15-v41','artifactId':8560331055,'adapterSha256':'31bead5f59982d8e321517fd235b81f992f83d2e3792a55690c2a841ff0e28f8'},'method':'failure-directed minimal SFT continuation from champion','trainRows':len(tr),'validationRows':len(va),'epochs':1,'steps':len(tb),'learningRate':a.lr,'maxLength':a.max_length,'seed':a.seed,'beforeValidationLoss':before,'afterValidationLoss':after,'relativeValidationImprovement':(before-after)/before,'meanTrainLoss':sum(losses)/len(losses),'parameterDeltaL2':delta,'adapterBytes':weights.stat().st_size,'adapterSha256':sha(weights),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'runtime':{'seconds':time.time()-started,'peakRssMb':rss_mb(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')},'rollback':'hector-asi-qwen15-v41'}
