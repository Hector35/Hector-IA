...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--proxy',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--epochs',type=int,default=2);p.add_argument('--lr',type=float,default=8e-7);p.add_argument('--seed',type=int,default=3069);p.add_argument('--max-length',type=int,default=144);p.add_argument('--eos-lambda',type=float,default=.08);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def cases(path,expected):
 r=[json.loads(x) for x in Path(path).read_text().splitlines() if x.strip()];assert len(r)==expected and len({x['id'] for x in r})==len(r);return r
def base():
...
def main():
 ...
 tr,va,px,hi=supervised(a.train),supervised(a.validation),cases(a.proxy,12),cases(a.hidden,18);groups=[{norm(x['prompt']) for x in z} for z in (tr,va,px,hi)]
 for i in range(4):
  for j in range(i+1,4):assert not groups[i]&groups[j]
 assert sum(x['group']=='focal' for x in tr)==8 and sum(x['group']=='replay' for x in tr)==16
 ...
 b=base();champ=PeftModel.from_pretrained(b,src,is_trainable=False);before=meanloss(champ,vb,a.eos_lambda);bp=behavior(champ,tok,px);bh=behavior(champ,tok,hi);del champ,b;gc.collect()
 ...
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for batch in tb:
   opt.zero_grad(set_to_none=True);loss=ce(m,batch,a.eos_lambda);loss.backward();torch.nn.utils.clip_grad_norm_(pars,.15);opt.step();ls.append(float(loss.detach()))
 ...
 alphas=[.10,.20,.30,.45,.65,.85,1.0];candidates=[]
 for alpha in alphas:
  ...
  interpolate(w0,raw/'adapter_model.safetensors',d/'adapter_model.safetensors',alpha);b=base();mm=PeftModel.from_pretrained(b,d,is_trainable=False);v=meanloss(mm,vb,a.eos_lambda);pr=behavior(mm,tok,px);candidates.append({'objective':v,'alpha':alpha,'dir':d,'proxy':pr});del mm,b;gc.collect()
 viable=[x for x in candidates if x['proxy']['criticalPassed']==x['proxy']['criticalTotal'] and x['proxy']['passed']>=bp['passed']]
 pool=viable or candidates;chosen=max(pool,key=lambda x:(x['proxy']['passed'],x['proxy']['criticalPassed'],-x['objective']));best,alpha,bestdir=chosen['objective'],chosen['alpha'],chosen['dir'];final.mkdir(parents=True,exist_ok=True)
 for p in bestdir.iterdir():
  if p.is_file():(final/p.name).write_bytes(p.read_bytes())
 b=base();m=PeftModel.from_pretrained(b,final,is_trainable=False);ah=behavior(m,tok,hi);wf=final/'adapter_model.safetensors';lat=ah['meanLatencyMs']/max(1,bh['meanLatencyMs']);tl=ah['meanTokens']/max(1,bh['meanTokens']);g={'validationImproved':best<before,'proxyNonRegression':chosen['proxy']['passed']>=bp['passed'],'proxyCriticalPerfect':chosen['proxy']['criticalPassed']==chosen['proxy']['criticalTotal'],'behavioralImprovement':ah['score']>bh['score'],'criticalPerfect':ah['criticalPassed']==ah['criticalTotal'],'noCriticalRegression':ah['criticalPassed']>=bh['criticalPassed'],'latencyAcceptable':lat<=1.15,'tokenLengthAcceptable':tl<=1.05};g['eligibleForPromotion']=all(g.values());metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v69-behavior-selected','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'response-only LoRA with independent behavioral-proxy checkpoint selection','dataset':{'trainRows':len(tr),'focalRows':8,'replayRows':16,'validationRows':len(va),'proxyRows':len(px),'hiddenRows':len(hi),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'proxySha256':sha(a.proxy),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lr':a.lr,'eosLambda':a.eos_lambda,'seed':a.seed,'maxLength':a.max_length,'alphas':alphas,'selectedAlpha':alpha},'baseline':{'validationObjective':before,'selectionProxy':bp,'behavioral':bh},'candidate':{'validationObjective':best,'selectionProxy':chosen['proxy'],'selectionGrid':[{'alpha':x['alpha'],'objective':x['objective'],'proxyPassed':x['proxy']['passed'],'proxyCriticalPassed':x['proxy']['criticalPassed']} for x in candidates],'behavioral':ah,'latencyRatio':lat,'tokenRatio':tl,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-start,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}};(root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False)+'\n');print(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False))
