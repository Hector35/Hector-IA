...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--epochs',type=int,default=2);p.add_argument('--lr',type=float,default=1.0e-6);p.add_argument('--seed',type=int,default=3066);p.add_argument('--max-length',type=int,default=144);p.add_argument('--distill-lambda',type=float,default=.85);p.add_argument('--replay-ce',type=float,default=.25);p.add_argument('--anchor-lambda',type=float,default=.02);p.add_argument('--eos-lambda',type=float,default=.08);p.add_argument('--first-layer',type=int,default=12);p.add_argument('--last-layer',type=int,default=27);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def enc(tok,row,n):
 pre=tok.apply_chat_template([{'role':'system','content':SYS},{'role':'user','content':row['prompt']}],tokenize=False,add_generation_prompt=True);full=pre+row['chosen']+tok.eos_token;pl=len(tok(pre,add_special_tokens=False)['input_ids']);b=tok(full,return_tensors='pt',add_special_tokens=False,truncation=True,max_length=n);lab=b['input_ids'].clone();lab[:,:min(pl,lab.shape[1])]=-100;assert not torch.all(lab==-100);b['labels']=lab;b['weight']=float(row.get('weight',1.0));return b
def ce_loss(model,b,eos_l):
 o=model(input_ids=b['input_ids'],attention_mask=b['attention_mask']);lg=o.logits[:,:-1];la=b['labels'][:,1:];mask=la!=-100;ce=torch.nn.functional.cross_entropy(lg[mask],la[mask]);last=torch.where(mask[0])[0][-1];eos=torch.nn.functional.cross_entropy(lg[:,last],la[:,last]);return b['weight']*(ce+eos_l*eos)
def meanloss(model,bs,eos_l):
 model.eval();v=[]
 with torch.no_grad():
  for b in bs:v.append(float(ce_loss(model,b,eos_l)))
 return sum(v)/len(v)
...
def main():
 ...
 tok=AutoTokenizer.from_pretrained(src,use_fast=True);tok.pad_token=tok.pad_token or tok.eos_token;focal=[enc(tok,x,a.max_length) for x in tr if x['group']=='focal'];replay=[enc(tok,x,a.max_length) for x in tr if x['group']=='replay'];vb=[enc(tok,x,a.max_length) for x in va]
 ...
 with torch.no_grad():
  for x in replay:refs.append(champ(input_ids=x['input_ids'],attention_mask=x['attention_mask']).logits[:,:-1].to(torch.float16).cpu())
 del champ,b;gc.collect();b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);active=[]
 for n,p in m.named_parameters():
  keep='lora_' in n and any(f'.layers.{i}.' in n for i in range(a.first_layer,a.last_layer+1));p.requires_grad=keep
  if keep:active.append(n)
 pars=[p for p in m.parameters() if p.requires_grad];assert pars and active;anchors={n:p.detach().clone() for n,p in m.named_parameters() if p.requires_grad};opt=AdamW(pars,lr=a.lr,weight_decay=.01,foreach=False);root=Path(a.out);bestdir=root/'best-adapter';final=root/'adapter';root.mkdir(parents=True,exist_ok=True);best=float('inf');hist=[]
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for i in range(max(len(focal),len(replay))):
   opt.zero_grad(set_to_none=True);fb=focal[i%len(focal)];rb=replay[i%len(replay)];lf=ce_loss(m,fb,a.eos_lambda);o=m(input_ids=rb['input_ids'],attention_mask=rb['attention_mask']).logits[:,:-1];ref=refs[i%len(replay)].to(o.dtype);mask=rb['labels'][:,1:]!=-100;kl=torch.nn.functional.kl_div(torch.log_softmax(o[mask],-1),torch.softmax(ref[mask],-1),reduction='batchmean');lrp=ce_loss(m,rb,a.eos_lambda);anchor=sum(torch.mean((p-anchors[n])**2) for n,p in m.named_parameters() if p.requires_grad);loss=lf+a.distill_lambda*kl+a.replay_ce*lrp+a.anchor_lambda*anchor;loss.backward();torch.nn.utils.clip_grad_norm_(pars,.25);opt.step();ls.append(float(loss.detach()))
