...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--epochs',type=int,default=2);p.add_argument('--lr',type=float,default=1e-6);p.add_argument('--seed',type=int,default=3064);p.add_argument('--max-length',type=int,default=144);p.add_argument('--eos-lambda',type=float,default=.10);p.add_argument('--first-layer',type=int,default=24);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def loss(model,b,eos_l):
 o=model(**b);lg=o.logits[:,:-1];la=b['labels'][:,1:];mask=la!=-100;ce=torch.nn.functional.cross_entropy(lg[mask],la[mask]);last=torch.where(mask[0])[0][-1];eos=torch.nn.functional.cross_entropy(lg[:,last],la[:,last]);return ce+eos_l*eos
...
def main():
 ...
 tr,va,hi=rows(a.train),rows(a.validation),hidden(a.hidden);ss=[{norm(x['prompt']) for x in z} for z in (tr,va,hi)];assert not ss[0]&ss[1] and not ss[0]&ss[2] and not ss[1]&ss[2];assert sum(x['group']=='focal' for x in tr)==6 and sum(x['group']=='replay' for x in tr)==12
 tok=AutoTokenizer.from_pretrained(src,use_fast=True);tok.pad_token=tok.pad_token or tok.eos_token;tb=[enc(tok,x,a.max_length) for x in sorted(tr,key=lambda x:(x['group']!='replay',x['id']))];vb=[enc(tok,x,a.max_length) for x in va]
 ...
 b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);active=[]
 for n,p in m.named_parameters():
  keep='lora_' in n and any(f'.layers.{i}.' in n for i in range(a.first_layer,28));p.requires_grad=keep
 ...
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for batch in tb:
   opt.zero_grad(set_to_none=True);l=loss(m,batch,a.eos_lambda);l.backward();torch.nn.utils.clip_grad_norm_(pars,.35);opt.step();ls.append(float(l.detach()))
