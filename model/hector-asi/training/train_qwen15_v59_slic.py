...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--seed',type=int,default=3059);p.add_argument('--epochs',type=int,default=3);p.add_argument('--lr',type=float,default=2e-6);p.add_argument('--max-length',type=int,default=160);p.add_argument('--margin',type=float,default=.12);p.add_argument('--rank-lambda',type=float,default=.55);p.add_argument('--anchor',type=float,default=.03);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def refs(model,bs):
 ...
 with torch.no_grad():
  for _,c,u in bs:
   cl,_=seq(model,c);rl,_=seq(model,u);out.append(float((cl-rl).cpu()))
 return out
def objective(model,bs,ref,marg,lam):
 model.eval();v=[]
 with torch.no_grad():
  for (r,c,u),rm in zip(bs,ref):
   cl,ce=seq(model,c);rl,_=seq(model,u);rank=F.relu(torch.tensor(rm+marg)- (cl-rl))*float(r.get('weight',1));v.append(float((ce+lam*rank).cpu()))
 return sum(v)/len(v)
...
def main():
 ...
 b=base();champ=PeftModel.from_pretrained(b,src,is_trainable=False);trref,vref=refs(champ,tb),refs(champ,vb);before=objective(champ,vb,vref,a.margin,a.rank_lambda);bb=behavior(champ,tok,hi);del champ,b;gc.collect()
