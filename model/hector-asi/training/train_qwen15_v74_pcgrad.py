...
import argparse,gc,hashlib,json,os,platform,re,resource,time,unicodedata
...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--epochs',type=int,default=3);p.add_argument('--lr',type=float,default=3e-7);p.add_argument('--seed',type=int,default=3074);p.add_argument('--max-length',type=int,default=144);p.add_argument('--eos-lambda',type=float,default=.05);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def rows(path):
 r=[json.loads(x) for x in Path(path).read_text().splitlines() if x.strip()];assert r and len({x['id'] for x in r})==len(r);assert all(x.get('verified') is True and x.get('provenance')=='author-generated' and x.get('license')=='Apache-2.0-compatible' for x in r);return r
def hidden(path):r=[json.loads(x) for x in Path(path).read_text().splitlines() if x.strip()];assert len(r)==18;return r
def base():m=AutoModelForCausalLM.from_pretrained(MODEL,revision=REV,torch_dtype=torch.float32,low_cpu_mem_usage=True,attn_implementation='eager');m.config.use_cache=False;return m
...
def meanloss(model,batches,eos_l):
 model.eval();v=[]
 with torch.no_grad():
  for b in batches:v.append(float(objective(model,b,eos_l)))
 return sum(v)/len(v)
...
def gradvec(params):return torch.cat([(p.grad if p.grad is not None else torch.zeros_like(p)).reshape(-1) for p in params])
def setgrad(params,v):
 off=0
 for p in params:n=p.numel();p.grad=v[off:off+n].view_as(p).clone();off+=n
def main():
 ...
 tr,va,hi=rows(a.train),rows(a.validation),hidden(a.hidden);ss=[{norm(x['prompt']) for x in z} for z in (tr,va,hi)];assert not ss[0]&ss[1] and not ss[0]&ss[2] and not ss[1]&ss[2];f=[x for x in tr if x['group']=='focal'];r=[x for x in tr if x['group']=='replay'];assert len(f)==16 and len(r)==20
 tok=AutoTokenizer.from_pretrained(src,use_fast=True);tok.pad_token=tok.pad_token or tok.eos_token;fb=[encode(tok,x,a.max_length) for x in f];rb=[encode(tok,x,a.max_length) for x in r];vb=[encode(tok,x,a.max_length) for x in va]
 ...
 b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);params=[]
 for n,p in m.named_parameters():
  layer=re.search(r'layers\.(\d+)\.',n);idx=int(layer.group(1)) if layer else -1;keep=14<=idx<=27 and ('lora_A' in n or 'lora_B' in n);p.requires_grad=keep
  if keep:params.append(p)
 assert params;opt=AdamW(params,lr=a.lr,weight_decay=.03,foreach=False);root=Path(a.out);final=root/'adapter';root.mkdir(parents=True,exist_ok=True);hist=[]
 for ep in range(1,a.epochs+1):
  m.train();ls=[];conflicts=0
  for i,bf in enumerate(fb):
   br=rb[(i+(ep-1)*len(fb))%len(rb)];opt.zero_grad(set_to_none=True);objective(m,bf,a.eos_lambda).backward();gf=gradvec(params).detach();opt.zero_grad(set_to_none=True);objective(m,br,a.eos_lambda).backward();gr=gradvec(params).detach();dot=torch.dot(gf,gr)
   if dot<0:
    gf0=gf;gr0=gr;gf=gf0-dot*gr0/(torch.dot(gr0,gr0)+1e-12);gr=gr0-dot*gf0/(torch.dot(gf0,gf0)+1e-12);conflicts+=1
   g=(gf+gr)/2;setgrad(params,g);torch.nn.utils.clip_grad_norm_(params,.08);opt.step();ls.append(float(torch.linalg.vector_norm(g)))
  hist.append({'epoch':ep,'meanGradientNorm':sum(ls)/len(ls),'projectedConflicts':conflicts,'peakRssMb':rss()})
 m.save_pretrained(final,safe_serialization=True);tok.save_pretrained(final);after=meanloss(m,vb,a.eos_lambda);ab=behavior(m,tok,hi);wf=final/'adapter_model.safetensors';lat=ab['meanLatencyMs']/max(1,bb['meanLatencyMs']);tl=ab['meanTokens']/max(1,bb['meanTokens']);g={'validationImproved':after<before,'behavioralImprovement':ab['score']>bb['score'],'criticalPerfect':ab['criticalPassed']==ab['criticalTotal'],'noCriticalRegression':ab['criticalPassed']>=bb['criticalPassed'],'latencyAcceptable':lat<=1.15,'tokenLengthAcceptable':tl<=1.05};g['eligibleForPromotion']=all(g.values());metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v74-pcgrad','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'symmetric PCGrad task balancing over focal and replay gradients, lora_A/B layers 14-27','dataset':{'trainRows':len(tr),'focalRows':len(f),'replayRows':len(r),'validationRows':len(va),'hiddenRows':len(hi),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lr':a.lr,'eosLambda':a.eos_lambda,'seed':a.seed,'maxLength':a.max_length,'trainable':'lora_A and lora_B layers 14-27','gradientClip':.08,'weightDecay':.03},'baseline':{'validationObjective':before,'behavioral':bb},'candidate':{'validationObjective':after,'behavioral':ab,'latencyRatio':lat,'tokenRatio':tl,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-start,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}};(root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False)+'\n');print(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False))
