...
def main():
 ...
 tr,va,hi=rows(a.train),rows(a.validation),hidden(a.hidden);ss=[{norm(x['prompt']) for x in z} for z in (tr,va,hi)];assert not ss[0]&ss[1] and not ss[0]&ss[2] and not ss[1]&ss[2];assert sum(x['group']=='focal' for x in tr)==8 and sum(x['group']=='replay' for x in tr)==20
 ...
 b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);pa=[];pb=[]
 for n,p in m.named_parameters():
  layer=re.search(r'layers\.(\d+)\.',n);idx=int(layer.group(1)) if layer else -1;keep_b=16<=idx<=27 and 'lora_B' in n;keep_a=22<=idx<=27 and 'lora_A' in n;p.requires_grad=keep_a or keep_b
  if keep_b:pb.append(p)
  if keep_a:pa.append(p)
 assert pa and pb;opt=AdamW([{'params':pb,'lr':a.lr_b,'weight_decay':.03},{'params':pa,'lr':a.lr_a,'weight_decay':.05}],foreach=False);root=Path(a.out);final=root/'adapter';root.mkdir(parents=True,exist_ok=True);hist=[]
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for batch in tb:
   opt.zero_grad(set_to_none=True);loss=objective(m,batch,a.eos_lambda);loss.backward();torch.nn.utils.clip_grad_norm_(pb+pa,.06);opt.step();ls.append(float(loss.detach()))
  hist.append({'epoch':ep,'trainObjective':sum(ls)/len(ls),'peakRssMb':rss()})
 m.save_pretrained(final,safe_serialization=True);tok.save_pretrained(final);after=meanloss(m,vb,a.eos_lambda);ab=behavior(m,tok,hi);wf=final/'adapter_model.safetensors';lat=ab['meanLatencyMs']/max(1,bb['meanLatencyMs']);tl=ab['meanTokens']/max(1,bb['meanTokens']);g={'validationImproved':after<before,'behavioralImprovement':ab['score']>bb['score'],'criticalPerfect':ab['criticalPassed']==ab['criticalTotal'],'noCriticalRegression':ab['criticalPassed']>=bb['criticalPassed'],'latencyAcceptable':lat<=1.15,'tokenLengthAcceptable':tl<=1.05};g['eligibleForPromotion']=all(g.values());metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v72-asymmetric','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'asymmetric dual-factor LoRA: lora_B layers 16-27 plus low-rate lora_A layers 22-27','dataset':{'trainRows':len(tr),'focalRows':8,'replayRows':20,'validationRows':len(va),'hiddenRows':len(hi),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lrB':a.lr_b,'lrA':a.lr_a,'eosLambda':a.eos_lambda,'seed':a.seed,'maxLength':a.max_length,'trainable':'lora_B layers 16-27; lora_A layers 22-27','gradientClip':.06},'baseline':{'validationObjective':before,'behavioral':bb},'candidate':{'validationObjective':after,'behavioral':ab,'latencyRatio':lat,'tokenRatio':tl,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-start,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}};(root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False)+'\n');print(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False))
