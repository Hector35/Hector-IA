...
def cli():
 p=argparse.ArgumentParser();p.add_argument('--source-adapter',required=True);p.add_argument('--train',required=True);p.add_argument('--validation',required=True);p.add_argument('--hidden',required=True);p.add_argument('--out',required=True);p.add_argument('--epochs',type=int,default=2);p.add_argument('--lr',type=float,default=1.2e-6);p.add_argument('--seed',type=int,default=3063);p.add_argument('--max-length',type=int,default=144);p.add_argument('--focal-weight',type=float,default=.35);p.add_argument('--eos-lambda',type=float,default=.12);return p.parse_args()
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def loss(model,b,eos_l):
 o=model(**b);lg=o.logits[:,:-1];la=b['labels'][:,1:];mask=la!=-100;tokloss=torch.nn.functional.cross_entropy(lg[mask],la[mask]);last=torch.where(mask[0])[0][-1];eosloss=torch.nn.functional.cross_entropy(lg[:,last],la[:,last]);return tokloss+eos_l*eosloss
def meanloss(model,bs,eos_l):
 model.eval();v=[]
 with torch.no_grad():
  for b in bs:v.append(float(loss(model,b,eos_l)))
 return sum(v)/len(v)
...
def main():
 ...
 tr,va,hi=rows(a.train),rows(a.validation),hidden(a.hidden);ss=[{norm(x['prompt']) for x in z} for z in (tr,va,hi)];assert not ss[0]&ss[1] and not ss[0]&ss[2] and not ss[1]&ss[2];f=[x for x in tr if x['group']=='focal'];r=[x for x in tr if x['group']=='replay'];assert len(f)==6 and len(r)==10
 tok=AutoTokenizer.from_pretrained(src,use_fast=True);tok.pad_token=tok.pad_token or tok.eos_token;fb=[enc(tok,x,a.max_length) for x in f];rb=[enc(tok,x,a.max_length) for x in r];vb=[enc(tok,x,a.max_length) for x in va]
 ...
 b=base();m=PeftModel.from_pretrained(b,src,is_trainable=True);pars=[p for p in m.parameters() if p.requires_grad];opt=AdamW(pars,lr=a.lr,weight_decay=.01,foreach=False);root=Path(a.out);bestdir=root/'best-adapter';final=root/'adapter';root.mkdir(parents=True,exist_ok=True);best=float('inf');hist=[];conflicts=0
 for ep in range(1,a.epochs+1):
  m.train();ls=[]
  for i,br in enumerate(rb):
   bf=fb[i%len(fb)];opt.zero_grad(set_to_none=True);lf=loss(m,bf,a.eos_lambda);gf=torch.autograd.grad(lf,pars,retain_graph=False,allow_unused=False);lr=loss(m,br,a.eos_lambda);gr=torch.autograd.grad(lr,pars,retain_graph=False,allow_unused=False);dot=sum(torch.sum(x*y) for x,y in zip(gf,gr));nf=sum(torch.sum(x*x) for x in gf);nr=sum(torch.sum(y*y) for y in gr);scale=torch.sqrt((nr+1e-12)/(nf+1e-12));alpha=a.focal_weight
   if dot<0:alpha=min(alpha,float((nr/(nr-dot+1e-12)).clamp(0.0,1.0)));conflicts+=1
   for p,x,y in zip(pars,gf,gr):p.grad=alpha*scale*x+(1-alpha)*y
   torch.nn.utils.clip_grad_norm_(pars,.5);opt.step();ls.append(float(alpha*lf.detach()+(1-alpha)*lr.detach()))
  v=meanloss(m,vb,a.eos_lambda);hist.append({'epoch':ep,'trainObjective':sum(ls)/len(ls),'validationObjective':v,'gradientConflicts':conflicts,'peakRssMb':rss()})
  if v<best:best=v;m.save_pretrained(bestdir,safe_serialization=True);tok.save_pretrained(bestdir)
 del m,b,opt,pars;gc.collect();b=base();m=PeftModel.from_pretrained(b,bestdir,is_trainable=False);ab=behavior(m,tok,hi);m.save_pretrained(final,safe_serialization=True);tok.save_pretrained(final);wf=final/'adapter_model.safetensors';lat=ab['meanLatencyMs']/max(1,bb['meanLatencyMs']);tl=ab['meanTokens']/max(1,bb['meanTokens']);g={'validationImproved':best<before,'behavioralImprovement':ab['score']>bb['score'],'criticalPerfect':ab['criticalPassed']==ab['criticalTotal'],'noCriticalRegression':ab['criticalPassed']>=bb['criticalPassed'],'latencyAcceptable':lat<=1.15,'tokenLengthAcceptable':tl<=1.05};g['eligibleForPromotion']=all(g.values());metrics={'schemaVersion':1,'experimentId':'hector-asi-qwen15-v63-cagrad','status':'candidate-weights-generated','baseModel':MODEL,'baseRevision':REV,'sourceRuntime':'hector-asi-qwen15-v41','method':'CAGrad-lite continual LoRA with norm-balanced focal/replay gradients and adaptive focal shrinkage on conflict','dataset':{'trainRows':len(tr),'focalRows':len(f),'replayRows':len(r),'validationRows':len(va),'hiddenRows':len(hi),'trainSha256':sha(a.train),'validationSha256':sha(a.validation),'hiddenSha256':sha(a.hidden),'license':'author-generated Apache-2.0-compatible','privateUserData':False,'crossSplitOverlap':0},'hyperparameters':{'epochs':a.epochs,'lr':a.lr,'focalWeight':a.focal_weight,'eosLambda':a.eos_lambda,'seed':a.seed,'maxLength':a.max_length},'baseline':{'validationObjective':before,'behavioral':bb},'candidate':{'validationObjective':best,'behavioral':ab,'latencyRatio':lat,'tokenRatio':tl,'adapterSha256':sha(wf),'adapterBytes':wf.stat().st_size,'history':hist,'gradientConflicts':conflicts},'gates':g,'rollback':'hector-asi-qwen15-v41','runtime':{'seconds':time.time()-start,'peakRssMb':rss(),'python':platform.python_version(),'torch':torch.__version__,'hardware':platform.platform(),'githubSha':os.getenv('GITHUB_SHA'),'githubRunId':os.getenv('GITHUB_RUN_ID')}};(root/'metrics.json').write_text(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False)+'\n');print(json.dumps(metrics,indent=2,sort_keys=True,ensure_ascii=False))
